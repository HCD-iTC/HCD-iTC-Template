...
|FFC-RS|FFC-RS - Rejection Sampling a|Static domain parameters approved for [selection:
...
|KCDSA|KCDSA a|Domain parameters generation with (L, N) = [selection: (2048, 224), (2048, 256), (3076, 256)] bits|ISO/IEC 14888-3:2018 (Subclause 6.3) [KCDSA]

|EC-KCDSA|EC-KCDSA a|Elliptic Curves [selection: P-224, B-233, K-233, P-256, B-283, K-283]|ISO/IEC 14888-3:2018 (Subclause 6.7) [EC-KCDSA],

NIST SP800-186 (Section 3) [NIST Curves]
...
_When generating ECC key pairs for digital signature generation and if “ECDSA” or "EC-KCDSA" are claimed in FCS_COP.1/SigGen, then “ECC–ERB” or “ECC–RS” must be claimed. The sizes of the private key, which is a scalar, and the public key, which is a point on the elliptic curve, are determined by the choice of the curve._ 
...
* [selection: _128 bits, 192 bits, 256 bits, 512 bits_],
...
*FCS_COP.1.1/SigGen* The TSF shall perform _digital signature generation and verification_ in accordance with a specified cryptographic algorithm [selection: _cryptographic algorithm_] and cryptographic key sizes [selection: _cryptographic key sizes_] that meet the following: [selection: _list of standards_].
...
|RSASSA-PKCS1-v1_5
|Modulus of size [selection: _2048, 3072, 4096, 6144, 8192_] bits and hash [selection: _SHA-256, SHA-384, SHA-512_] 

*_RSA Digital Signature Algorithm and cryptographic key sizes (modulus) [assignment: 2048 bits or greater],_*

|RFC 8017 (Section 8.2) [PKCS #1 v2.2], FIPS PUB 186-5 (Section 5.4) [RSASSA-PKCS1-v1_5]
...
|ECDSA
|Elliptic Curve [selection: _P-256, P-384, P-521_], per-message secret number generation [selection: _extra random bits, rejection sampling, deterministic_] and hash function using [selection: _SHA-256, SHA-384, SHA-512_]

*_Elliptic Curve Digital Signature Algorithm and cryptographic key sizes [assignment: 256 bits or greater],_*

|[selection: _ISO/IEC 14888-3:2018 (Subclause 6.6), FIPS PUB 186-5 (Sections 6.3.1, 6.4.1)_][ECDSA], 

NIST SP 800-186 (Section 3) [NIST Curves]

*_FIPS PUB 186-4, “Digital Signature Standard (DSS)”, Section 6 and Appendix D, Implementing “NIST curves” [selection: P-256, P-384, P-521]; ISO/IEC 14888-3, Section 6.4_*

|KCDSA
|hash function using [selection: _SHA-224, SHA-256, SHA-384, SHA-512_]
...
|EC-KCDSA
|Elliptic Curve [selection: _P-224, P-256, B-233, B-283, K-233, K-283] using hash [selection: _SHA-224, SHA-256, SHA-384, SHA-512_]
...
*FCS_COP.1.1/Hash:* The TSF shall perform _cryptographic hashing_ in accordance with a specified cryptographic algorithm [selection: _SHA-1, SHA-256, SHA-384, SHA-512_] that meets the following: [selection: ISO/IEC 10118-3:2004 [SHA], FIPS PUB 180-4 [SHA]].
...
_The hash selection should be consistent with the overall strength of the algorithm used for signature generation. For example, the TOE should choose SHA-256 for 2048-bit RSA or ECC with P-256; SHA-384 for 3072-bit RSA, 4096-bit RSA, or ECC with P-384; and SHA-512 for ECC with P-521. The ST author selects the standard based on the algorithms selected._
...
|KW|[selection: AES, SEED, LEA] in KW mode|[selection: (AES, SEED, LEA) 128, (AES, LEA) 192, (AES, LEA) 256] bits
|
[selection: _ISO/IEC 19772:2020 (clause 6), NIST SP 800-38F (Section 6.2)_] [KW mode]

[selection: _ISO/IEC 18033-3:2010 (Subclause 5.2), FIPS PUB 197_] [AES]


|KWP|[selection: AES, SEED, LEA] in KWP mode|[selection: (AES, SEED, LEA) 128, (AES, LEA) 192, (AES, LEA) 256] bits
|
NIST SP 800-38F (Section 6.3) [KWP mode]

[selection: _ISO/IEC 18033-3:2010 (Subclause 5.2), FIPS PUB 197_] [AES]

|CCM|[selection: AES, SEED, LEA] in CCM mode with unpredictable, non-repeating nonce, minimum size of 64 bits|[selection: (AES, SEED, LEA) 128, (AES, LEA) 192, (AES, LEA) 256] bits
|
[selection: _ISO/IEC 19772:2020 (clause 7), NIST SP 800-38C CCM_],

[selection: _ISO/IEC 18033-3:2010 (Subclause 5.2), FIPS PUB 197_] [AES]

|GCM|[selection: AES, SEED, LEA] in GCM mode with non-repeating IVs using [selection: _deterministic, RBG-based_], IV construction; the tag must be of length [selection: _96, 104, 112, 120, 128_] bits. |[selection: (AES, SEED, LEA) 128, (AES, LEA) 192, (AES, LEA) 256] bits
|
...
_The SEED algorithm supports keys of size 128 bits only._
...
*FCS_TLSC_EXT.1.1* The TSF shall implement [selection: _TLS 1.3 (RFC 8446), TLS 1.2 (RFC 5246), TLS 1.1 (RFC 4346)_] and reject all other TLS and SSL versions.  The TLS implementation will support the following ciphersuites: [selection: _select supported ciphersuites from List 1_] and no other ciphersuites.
...
*FCS_TLSS_EXT.1.1* The TSF shall implement [selection: _TLS 1.3 (RFC 8446), TLS 1.2 (RFC 5246), TLS 1.1 (RFC 4346)_] and reject all other TLS and SSL versions.  The TLS implementation will support the following ciphersuites: [selection: _select supported ciphersuites from List 1_] and no other ciphersuites.
...
*FCS_DTLSC_EXT.1.1* The TSF shall implement [selection: _DTLS 1.3 (RFC 9147), DTLS 1.2 (RFC 6347), DTLS 1.0 (RFC 4347)_] supporting the following ciphersuites: [selection: _select supported ciphersuites from List 1_] and no other ciphersuites.
...
*FCS_DTLSS_EXT.1.1* The TSF shall implement [selection: _DTLS 1.3 (RFC 9147), DTLS 1.2 (RFC 6347), DTLS 1.0 (RFC 4347)_] supporting the following ciphersuites: [selection: _select supported ciphersuites from List 1_] and no other ciphersuites.
...
• CCM: Nonce shall be non-repeating, unpredictable, and at least 64 bits in size.
...
*FCS_DTLSS_EXT.1.1* The TSF shall implement [selection: DTLS 1.3 (RFC 9147), DTLS 1.2 (RFC 6347), DTLS 1.0 (RFC 4347)] supporting the following ciphersuites: [assignment: _List of optional ciphersuites and reference to RFC in which each is defined_] and no other ciphersuites.].
