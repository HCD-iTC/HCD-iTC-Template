...
_In the case where an encrypted key is stored on media that is decrypted by a key stored in a protected storage device, the selection for "only store keys in non-volatile memory when wrapped, as specified in FCS_COP.1/KeyWrap, or encrypted, as specified in FCS_COP.1/KeyEnc or FCS_COP.1/KeyTransport" should be used. In this case, the key that the protected storage device generates and stores internally is considered to be the starting point of the key chain, so "the non-volatile memory where the key is stored on is located in a protected storage device" must also be selected as a means of protecting that key._
...
** _the non-volatile memory where the key is stored on is located in a protected storage device]_
