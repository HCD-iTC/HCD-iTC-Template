...
| 2.0-pre.3 | 20 July 2026 | Working Draft v2.0 #3
| 2.0-pre.4 | 30 July 2026 | Working Draft v2.0 #4
...
* [#CC5]#[CC5]# Common Criteria for Information Technology Security Evaluation, Part 5: Pre-defined packages of security requirements, CCMB-2022-11-005, CC:2022 Release 1, November 2022.
...
* [#CC-E&I]#[CC-E&I]# Errata and Interpretation for CC:2022 (Release 1) and CEM:2022 (Release 1), Version 1.2, 2025-10-15.

* [#SFR-CRYPTO]#[SFR-CRYPTO]# Specification of Functional Requirements for Cryptography, Version 1.0, January 31, 2025.

* [#SD]#[SD]# Supporting Document Mandatory Technical Document: Evaluation Activities for collaborative Protection Profile for Hardcopy Devices, v2.0-pre.4, Draft v2.0 #4, 30 July 2026.
...
* Errata and Interpretation for CC:2022 (Release 1) and CEM:2022 (Release 1), Version 1.2, 2025-10-15.
...
All cryptographic selections in the above packages must comply with the FCS_COP and FCS_CKM requirements of this cPP while utilizing this cPP's defined FCS_RBG_EXT.1.


=== Conformance to this Protection Profile
To claim conformance to this Protection Profile, the conforming Security Target must comply with all of the following rules:
...
.. For each auditable event type, based on the auditable event definitions of the functional components included in the PP, PP-Module, functional package or ST, *additional information specified in Table 3*, [assignment: _other audit relevant information_].
...
* _RSA schemes using cryptographic key sizes of 2048-bit or greater that meet the following: FIPS PUB 186-5, “Digital Signature Standard (DSS)”, Appendix A.1;_
* _ECC schemes using ‘NIST curves’ [selection: P-256, P-384, P-521] that meet the following: FIPS PUB 186-5, “Digital Signature Standard (DSS)”, Appendix A.2;_
...
* _Module-Lattice-Based Key-Encapsulation Mechanism Standard using the parameter set ML-KEM-1024 that meets the following: [FIPS 203, Module-Lattice-Based Key-Encapsulation Mechanism Standard]_
* _Module-Lattice-Based Digital Signature Standard using the parameter set ML-DSA-87 that meets the following [FIPS 204, Module-Lattice-Based Digital Signature Standard]_

] *_+++<del>and specified cryptographic key sizes [assignment: cryptographic key sizes] that meet the following: [assignment: list of standards]</del>+++_*.
...
*FCS_CKM.1.1/SKG Refinement:* The TSF shall generate *symmetric* cryptographic keys *_+++<del>in accordance with a specified cryptographic key generation algorithm</del>+++_* *using a* [*Random Bit Generator as specified in FCS_RBG_EXT.1*] and specified cryptographic key sizes [selection: _128 bits, 192 bits, 256 bits, 512 bits_] that meet the following: [selection: _ISO/IEC 18031:2011 (Clause 9) [DRBG], NIST SP 800-133 Rev.2 Section [selection: 6.1, 6.3]_].
...
*Dependencies:* +
	FCS_CKM.1/AKG Cryptographic Key Generation (Asymmetric Keys) +
	FCS_CKM.6 Timing and event of cryptographic key destruction, +
    [if selected: FCS_COP.1/KeyEncap Key Encapsulation]
======

*FCS_CKM.2.1 Refinement* The TSF shall *_+++<del>distribute</del>+++_* *perform* cryptographic *_+++<del>keys</del>+++_* *key establishment* in accordance with a specified cryptographic key *_+++<del>distribution</del>+++_* *establishment* method: [*selection:*

•	_RSA-based key establishment schemes that meet the following: [selection: RSAES-PKCS1-v1_5 as specified in Section 7.2 of RFC 8017, “Public-Key Cryptography Standards (PKCS) #1: RSA Cryptography Specifications Version 2.2”, NIST SP 800-56B Revision 2 (Section 8.3) [KAS2]];_
...
•	_FFC Schemes using “safe-prime” groups that meet the following: ‘NIST Special Publication 800-56A Revision 3, “Recommendation for Pair-Wise Key Establishment Schemes Using Discrete Logarithm Cryptography” and [selection: RFC 3526, RFC 7919];_
•	_Module-Lattice-based Key-Encapsulation Mechanisms (ML-KEM) that meet the following: NIST FIPS 203._

] *_+++<del>that meets the following: [assignment: list of standards]</del>+++_*.
...
_If ML-KEM is selected, FCS_COP.1/KeyEncap must be claimed.  The parameters used for the Module-Lattice-based Key-Encapsulation Mechanisms are specified in FCS_CKM.1.1/AKG and FCS_COP.1.1/KeyEncap._
...
*Dependencies:* +
	pass:[[][.line-through]#FDP_ITC.1 Import of user data without security attributes, or# +
	[.line-through]#FDP_ITC.2 Import of user data with security attributes, or# +
	FCS_CKM.1 Cryptographic key generation, or +
	FCS_CKM.5 Cryptographic key derivation{endsb}
======

*FCS_CKM.6.1* The TSF shall destroy [*plaintext cryptographic keys (including keying material)*] when [selection: _no longer needed, [assignment: other circumstances for key or keying material destruction]_].
...
*FCS_CKM.6.2 Refinement* The TSF shall destroy *plaintext* cryptographic keys and keying material specified by FCS_CKM.6.1 in accordance with a specified cryptographic key destruction method [selection:
...
* _For nonvolatile storage [selection:_
...
***** _[selection: single, [assignment: ST author defined multi-pass]] overwrite consisting of [selection: zeros, ones, pseudo-random pattern, a new value of a key of the same size, [assignment: some value that does not contain any CSP]] followed by a read-verify. If the read-verification of the overwritten data fails, the process shall be repeated up to [assignment: number of times to attempt overwrite] times, whereupon an error is returned._
***** _block erase]]_
...
*Dependencies:* +
	pass:[[][.line-through]#FDP_ITC.1 Import of user data without security attributes, or# +
	[.line-through]#FDP_ITC.2 Import of user data with security attributes, or# +
	FCS_CKM.1/SKG Cryptographic Key Generation (Symmetric Keys)[.line-through]#, or# +
	[.line-through]#FCS_CKM.5 Cryptographic key derivation#{endsb} +
	FCS_CKM.6 Timing and event of cryptographic key destruction
======
...
* _AES used in_ [selection:
** _CBC mode with non-repeating and unpredictable IVs_,
** _CTR mode with a non-repeating initial counter and no repeated use of counter values across multiple messages with the same secret key_,
** _XTS mode with unique tweak values that are consecutive non-negative integers starting at an arbitrary non-negative integer_,
** _GCM mode with non-repeating IVs using [selection: deterministic, RBG-based] IV construction; the tag must be of length [selection: 96, 104, 112, 120, 128] bits_,
** _CCM mode with unpredictable, non-repeating nonce, minimum size of 64 bits_,
** _KW mode_,
** _KWP mode_],
...
* _128 bits, 192 bits, 256 bits, 512 bits_,
...
* _ISO 18033-3_,
** [selection: _CBC as specified in ISO 10116, CTR as specified in ISO 10116, GCM as specified in ISO 19772_],
* _FIPS PUB 197 (AES)_,
** [selection: _CBC as specified in NIST SP 800-38A, CTR as specified in NIST SP 800-38A, XTS as specified in NIST SP 38-E, GCM as specified in NIST SP 800-38D, CCM as specified in NIST SP 800-38C, KW as specified in NIST SP 800-38F, KWP as specified in NIST SP 800-38F_],
...
* _Elliptic Curve KCDSA (EC-KCDSA) on [selection: NIST P-224, NIST P-256, NIST B-233, NIST B-283, NIST K-233, NIST K-283] using [selection: SHA-224, SHA-256, SHA-384, SHA-512] with key size of [selection: 224 bits, 256 bits]_

* _Module-Lattice-based Digital Signature Algorithm using parameter set ML-DSA-87_

* _Leighton-Micali Signature (LMS) scheme with parameters:_
** _Private key size = [selection: 192 bits with [selection: SHA-256/192, SHAKE256/192], 256 bits with [selection: SHA-256, SHAKE256]]_
** _Winternitz parameter = [selection: 1, 2, 4, 8]_
** _Tree height = [selection: 5, 10, 15, 20, 25]_

* _eXtended Merkle Signature Scheme (XMSS) with parameters:_
** _Private key size = [selection: 192 bits with [selection: SHA-256/192, SHAKE256/192], 256 bits with [selection: SHA-256, SHAKE256]]_
** _Tree height = [selection: 10, 16, 20]_
...
* _FIPS PUB 186-5, “Digital Signature Standard (DSS)”, Section 6 and Appendix A,_
...
* _FIPS PUB 186-4 (Appendix D.1.2, D.1.3), “Digital Signature Standard”,_
...
* _NIST FIPS 204 (Section 5.2)_

Case: LMS Schemes

* _RFC 8554, “Leighton-Micali Hash-Based Signatures”_
* _NIST SP 800-208, “Recommendation for Stateful Hash-Based Signature Schemes”_

Case: XMSS Schemes

* _RFC 8391, “XMSS: eXtended Merkle Signature Scheme”_
* _NIST SP 800-208, “Recommendation for Stateful Hash-Based Signature Schemes”_
...
*FMT_MOF.1.1:* The TSF shall restrict the ability to [selection: _determine the behaviour of, disable, enable, modify the behaviour of_] the functions [assignment: _list of functions_] to [*U.ADMIN*].
...
*FMT_MSA.1.1* The TSF shall enforce the [*User Data Access Control SFP*] to restrict the ability to [selection: __change_default, query, modify, delete, [assignment: other operations]__] the security attributes [assignment: _list of security attributes_] to [assignment: _the authorized identified roles_].
...
*FMT_MSA.3.1* The TSF shall enforce the [*User Data Access Control SFP*] to provide [selection, choose one of: _restrictive, permissive, [assignment: other property]_] default values for security attributes that are used to enforce the SFP.
...
*FMT_SMF.1.1:* The TSF shall be capable of performing the following management functions: [assignment: _list of management functions to be provided by the TSF_].
...
*Dependencies:* +
	[if selected: FCS_COP.1/Hash Cryptographic operation (Hash Algorithm)], +
	[if selected: FCS_COP.1/SigGen Cryptographic Operation (for signature generation/verification)], +
	[if selected: FCS_COP.1/KeyedHash Cryptographic Operation (for keyed-hash message authentication), and/or FCS_COP.1/CMAC Cryptographic Operation (for cipher-based message authentication)]
======
...
*Dependencies:* +
	[if selected: FCS_COP.1/SigGen Cryptographic Operation (for signature generation/verification)], +
	[if selected: FIA_X509_EXT.1 X.509 Certificate Validation], +
	[if selected: FCS_COP.1/Hash Cryptographic operation (Hash Algorithm)]
======
...
*FTP_ITC.1.1 Refinement:* The TSF shall *use* [selection: _IPsec, SSH, TLS, DTLS, TLS/HTTPS_] *to* provide *a trusted* communication channel between itself and *_+++<del>another trusted IT product</del>+++_* *authorized IT entities supporting the following capabilities: remote audit server,* [selection: __authentication server, [assignment: other capabilities]__] that is logically distinct from other communication channels and provides assured identification of its end points and protection of the channel data from *_+++<del>modification or</del>+++_* disclosure *and detection of modification of the channel data*.

*FTP_ITC.1.2 Refinement:* The TSF shall permit [selection: _the TSF,_ *_+++<del>another trusted IT product</del>+++_* *the authorized IT entities*] to initiate communication via the trusted channel.

*FTP_ITC.1.3 Refinement:* The TSF shall initiate communication via the trusted channel for *_+++<del>[assignment: list of functions for which a trusted channel is required]</del>+++_* [*remote audit server* [assignment: __and list of services for which the TSF is able to initiate communications__]].
...
*FTP_TRP.1.1/Admin Refinement:* The TSF shall *use* [selection, choose at least one of: _IPsec, SSH, TLS, DTLS, TLS/HTTPS_] *to* provide a *trusted* communication path between itself and [remote] *_+++<del>users</del>+++_* *administrators* that is logically distinct from other communication paths and provides assured identification of its end points and protection of the communicated data from [disclosure *and detection of modification of the communicated data*].

*FTP_TRP.1.2/Admin Refinement:* The TSF shall permit [remote *_+++<del>users</del>+++_* *administrators*] to initiate communication via the trusted path.

*FTP_TRP.1.3/Admin Refinement:* The TSF shall require the use of the trusted path for [initial *_+++<del>user</del>+++_* *administrator* authentication *[and all remote administration actions]*].
...
======
(selected with FDP_DSK_EXT.1.1, FPT_SBT_EXT.1.5, for O.STORAGE_ENCRYPTION, O.STRONG_CRYPTO) +
*Hierarchical to:* +
No other components.
...
* _ISO 18033-3_,
** [selection: _CBC as specified in ISO 10116, GCM as specified in ISO/IEC 19772, and XTS as specified in IEEE 1619_],
...
======
(selected with FCS_CKM.2.1 for O.COMMS_PROTECTION, O.STRONG_CRYPTO) +
*Hierarchical to:* +
No other components. +
...
pass:[[][.line-through]#FDP_ITC.1 Import of user data without security attributes, or# +
[.line-through]#FDP_ITC.2 Import of user data with security attributes, or# +
FCS_CKM.1/AKG Cryptographic Key Generation (Asymmetric Keys)[.line-through]#, or# +
[.line-through]#FCS_CKM.5 Cryptographic key derivation, or# +
[.line-through]#FCS_CKM_EXT.7 Cryptographic key agreement, or# +
[.line-through]#FCS_CKM_EXT.8 Password-based key derivation#] +
FCS_CKM.6 Timing and event of cryptographic key destruction, +
[.line-through]#FCS_OTV_EXT.1 One-Time Value#{endsb}.
======

*FCS_COP.1.1/KeyEncap:* The TSF shall perform key encapsulation in accordance with a specified cryptographic algorithm [selection: cryptographic algorithm] and cryptographic key sizes [selection: cryptographic algorithm parameters] that meet the following: [selection: list of standards].
...
*FCS_COP.1.1/KeyWrap Refinement:* The TSF shall perform [*key wrapping*] in accordance with a specified cryptographic algorithm [selection: _AES in the following modes [selection: KW, KWP, GCM, CCM], SEED in the following modes [selection: KW, KWP, GCM, CCM], LEA in the following modes [selection: KW, KWP, GCM, CCM]_] and the cryptographic key size [selection: _128 bits (AES, SEED, LEA), 192 bits (AES, LEA), 256 bits (AES, LEA)_] that meet the following: [selection:

Case: AES algorithm

* [selection:

** _ISO/IEC 18033-3 (AES), [selection: NIST SP 800-38F, ISO/IEC 19772, no other standards]_,
** _FIPS PUB 197 (AES), [selection: NIST SP 800-38F (KW,KWP), NIST SP 800-38D (GCM), NIST SP 800-38C (CCM)]_
...
* _AES used in [[selection: CBC, GCM] mode] and cryptographic key sizes [selection: 128 bits, 192 bits, 256 bits] that meet the following: *AES as specified in [selection: ISO/IEC 18033-3, FIPS PUB 197]*, [selection: CBC as specified in [selection: ISO/IEC 10116, NIST SP 800-38A], GCM as specified in [selection: ISO/IEC 19772, NIST SP 800-38D]]_,
...
*FCS_COP.1.1/KeyTransport Refinement:* The TSF shall perform [*key transport*] in accordance with a specified cryptographic algorithm [selection: 

* RSA in the following modes [selection: _KTS-OAEP, KTS-KEM-KWS_] and the cryptographic key size [selection: _2048 bits, 3072 bits, 4096 bits, 6144 bits, 8192 bits_] that meet the following: [NIST SP 800-56B, Revision 2],

* KEM-DEM Key-Transport Method that meets the following: [NIST SP 800-227 Section 5.2.1] using a key establishment algorithm [ML-KEM] as specified in FCS_CKM.2 and a key wrapping algorithm as specified in FCS_COP.1/KeyWrap]
...
*FCS_SMC_EXT.1.1* The TSF shall combine submasks using the following method [selection: _exclusive OR (XOR), SHA-256, SHA-384, SHA-512, HMAC-SHA-384, HMAC-SHA-512_] to generate an intermediary key, BEV or DEK.
...
=== Protected Communications
As indicated in the FTP requirements, there are several methods by which conformant TOEs can mitigate threats against compromise of the communication channel between administrators, other portions of the TOE, or external IT entities. One of the secure communication protocols (IPsec, SSH, TLS, DTLS, TLS/HTTPS) must be implemented in order to provide protected connectivity for (at a minimum) the audit server and remote administrators.
...
*Dependencies:* +
	[if selected: FIA_PSK_EXT.1 Extended: Pre-Shared Key Composition], +
	FCS_CKM.1/AKG Cryptographic Key Generation (Asymmetric Keys), +
	FCS_CKM.2 Cryptographic Key Establishment, +
	FCS_COP.1/DataEncryption Cryptographic Operation (Symmetric encryption/decryption), +
	FCS_COP.1/SigGen Cryptographic Operation (for signature generation/verification), +
	FCS_COP.1/Hash Cryptographic Operation (Hash Algorithm), +
	[if selected: FCS_COP.1/KeyedHash Cryptographic Operation (Keyed Hash Algorithm)], +
	FCS_RBG_EXT.1 Random Bit Generation
=====
...
_AES-GCM-128 and AES-GCM-256 may only be selected if IKEv2 is also selected, as there is no RFC defining AES-GCM for IKEv1._
...
_The intent of this element is to specify user authentication mechanism(s) that the TOE supports when acting as an SSH client. The TOE is required to implement the capability to generate user-based authentication keys in accordance with FCS_CKM.1 as specified by FMT_SMF.1 via “Ability to manage the cryptographic keys”. While no specific public key algorithms are mandatory to implement, the use of public key algorithms must be consistent with FCS_CKM.1, FCS_COP.1/Hash, and FCS_COP.1/SigGen._
...
==== FCS_COP.1/KeyedHash Cryptographic Operation (Keyed Hash Algorithm)
======
(selected with FCS_PCC_EXT.1 for O.STORAGE_ENCRYPTION, O.STRONG_CRYPTO, selected with FCS_IPSEC_EXT.1.4, FCS_SSHC_EXT.1.6, FCS_SSHS_EXT.1.6, FCS_TLSC_EXT, FCS_TLSS_EXT, FCS_DTLSC_EXT, FCS_DTLSS_EXT for O.COMMS_PROTECTION, O.STRONG_CRYPTO) +
*Hierarchical to:* +
No other components. +
...
======
(selected with FCS_IPSEC_EXT.1.13 for O.COMMS_PROTECTION) +
*Hierarchical to:* +
No other components.
...
*FIA_PSK_EXT.1.3* The TSF shall condition the text-based pre-shared keys by using [selection: _SHA-1, SHA-256, SHA-384, SHA-512_, [assignment: _method of conditioning text string_]] and be able to [selection: _use no other pre-shared keys; accept bit-based pre-shared keys; generate bit-based pre-shared keys using the random bit generator specified in FCS_RBG_EXT.1_].
...
*Dependencies:* +
 	[+++<del>+++*_FCS_CKM.2 Cryptographic key distribution, or_*+++</del>+++ +
	[if selected: FCS_COP.1/KeyedHash Cryptographic Operation (Keyed Hash Algorithm)], +
    [if selected: FCS_COP.1/CMAC Cryptographic Operation (for cipher-based message authentication)], +
	FCS_CKM.6 Timing and event of cryptographic key destruction +
	[if selected: FCS_RBG_EXT.1 Random Bit Generation]
======

*FCS_CKM.5.1* The TSF shall derive cryptographic keys [*intermediate key*] from [selection: _a RNG generated submask as specified in FCS_RBG_EXT.1, a conditioned password submask, imported submask_] in accordance with a specified key derivation algorithm [selection: _the keyed-hash functions specified in FCS_COP.1/KeyedHash, the cipher-based function specified in FCS_COP.1/CMAC_] and specified cryptographic key sizes [*at least of equivalent security strength (in number of bits) to the BEV or the DEK*] that meet the following: [selection: _NIST SP 800-108 [selection: KDF in Counter Mode, KDF in Feedback Mode, KDF in Double-Pipeline Iteration Mode], NIST SP 800-132, ISO/IEC 11770-6:2016 [selection: KPF2, KPF3, KPF4]_].
...
==== FCS_SNI_EXT.1 Extended: Cryptographic Operation (Salt and Nonce Generation)
======
(selected with FCS_PCC_EXT.1, FCS_CKM.5.1, for O.STORAGE_ENCRYPTION, O.STRONG_CRYPTO) +
*Hierarchical to:* +
	No other components +
...
_FIA_X509_EXT.1.2 applies to certificates that are used and processed by the TSF and restricts the certificates that may be added as trusted CA certificates._

_The ST author must include FIA_X509_EXT.1 in all instances except when only SSH is selected within FTP_ITC.1, and implementation is limited to public-key authentication that does not rely on X.509 certificates. Additionally, FIA_X509_EXT.1 must also be included if "X.509 Certificate" is selected in FPT_TUD_EXT.1.3._
...
*Dependencies:* +
	...
	[if selected: FCS_COP.1/KeyWrap Cryptographic operation (Key Wrapping)], +
	[if selected: FCS_COP.1/KeyEnc Cryptographic operation (Key Encryption) and/or FCS_COP.1/KeyTransport Cryptographic operation (Key Transport)], +
	[if selected: FCS_SMC_EXT.1 Extended: Submask Combining]
======
...
*FCS_KYC_EXT.1.1* The TSF shall maintain a key chain of: [selection: _one, using a submask as the BEV or DEK; intermediate keys originating from one or more submask(s) to the BEV or DEK using the following method(s): [selection: key wrapping as specified in FCS_COP.1/KeyWrap, key combining as specified in FCS_SMC_EXT.1, key encryption as specified in FCS_COP.1/KeyEnc, key derivation as specified in FCS_CKM.5, key transport as specified in FCS_COP.1/KeyTransport]] while maintaining an effective strength of [selection: 128 bits, 256 bits_].
...
FCS_KYC_EXT.1.1 The TSF shall maintain a key chain of: [selection: _one, using a submask as the BEV or DEK; intermediate keys originating from one or more submask(s) to the BEV or DEK using the following method(s)_: [selection: _key wrapping as specified in FCS_COP.1/KeyWrap, key combining as specified in FCS_SMC_EXT.1, key encryption as specified in FCS_COP.1/KeyEnc, key derivation as specified in FCS_CKM.5, key transport as specified in FCS_COP.1/KeyTransport_]] while maintaining an effective strength of [selection: _128 bits, 256 bits_].
...
FCS_RBG_EXT.1 specifies deterministic random bit generation in accordance with ISO/IEC 18031:2011 using an identified DRBG mechanism, and requires the deterministic RBG to be seeded by at least one entropy source that provides a minimum amount of entropy at least equal to the greatest security strength of the keys and hashes that the RBG will generate. The cryptographic SFRs in this cPP that require random bits rely on FCS_RBG_EXT.1 for both the deterministic random bit generation mechanism and the entropy-source seeding requirement.

Common Criteria Part 2 includes a random bit generation component, FCS_RBG.1. FCS_RBG.1 carries a defined dependency structure, including dependencies on FCS_RBG.2 or FCS_RBG.3 for external or internal seeding, and dependencies on FPT_FLS.1, Failure with preservation of secure state, and FPT_TST.1, TSF self-testing. Use of FCS_RBG.1 would therefore require the cPP to include or justify additional dependent requirements that are broader than the random bit generation baseline established for conformant TOEs.

The intended cPP baseline is limited to the deterministic random bit generation mechanism, the applicable standard, and the entropy-source seeding requirement. FCS_RBG_EXT.1 states this baseline precisely as a single, self-contained component, without adding broader failure-state or TSF self-testing requirements that are not part of this random bit generation baseline. It is also referenced consistently by the cryptographic SFRs in this cPP that require random bits.

FCS_RBG_EXT.1 is therefore retained and placed in the FCS class.
...
==== FCS_SNI_EXT  Extended: Cryptographic Operation (Salt and Nonce Generation)

*Family Behaviour*

This family ensures that salts and nonces are well formed.
...
======
  +----------------------------------------------------+
  |                                                    |     +---+
  | FCS_SNI_EXT Cryptographic Operation (Salt and      +---->| 1 |
  | Nonce Generation)                                  |     +---+
  |                                                    |
  +----------------------------------------------------+
======

FCS_SNI_EXT.1 Cryptographic Operation (Salt and Nonce Generation), requires the generation of salts and nonces to be used by the cryptographic components of the TOE to be performed in the specified manner.
...
The TSF is required to ensure that the generation of salts and nonces to be used by the cryptographic components of the TOE is to be performed in the specified manner.

This extended component protects the communication data and storage data using cryptographic algorithms with specified Salt and Nonce Generation, and it is therefore placed in the FCS class with a single component.
...
======
  +----------------------------------------------------+
  |                                                    |     +---+
  | FCS_SSHC_EXT Extended: SSH Client Protocol         +---->| 1 |
  |                                                    |     +---+
  |                                                    |
  +----------------------------------------------------+
======
...
SSH is used by the SFRs to establish a trusted channel/path as a client, and the Common Criteria does not provide a suitable SFR for the SSH client protocol.

This extended component ensures the secure operation of the SSH client protocol, and it is therefore placed in the FCS class.
...
======
  +----------------------------------------------------+
  |                                                    |     +---+
  | FCS_SSHS_EXT Extended: SSH Server Protocol         +---->| 1 |
  |                                                    |     +---+
  |                                                    |
  +----------------------------------------------------+
======
...
SSH is used by the SFRs to establish a trusted channel/path as a server, and the Common Criteria does not provide a suitable SFR for the SSH server protocol.

This extended component ensures the secure operation of the SSH server protocol, and it is therefore placed in the FCS class.
...
======
    +------------------------------------------+
    |                                          |     +---+
    | FDP_UDU_EXT Document Unavailability      +---->| 1 |
    |                                          |     +---+
    +------------------------------------------+
======
...
FIA_PMG_EXT.1 specifies password management capabilities for User passwords, including permitted password composition using upper-case letters, lower-case letters, numbers, and a selectable set of special characters, as well as a minimum password length that is settable by an Administrator and capable of being configured to require 15 characters or greater.

The Common Criteria Part 2 component FIA_SOS.1, Verification of secrets, provides for verification that secrets meet a defined quality metric and can express password-composition requirements. However, FIA_SOS.1 does not clearly express the full password management capability required by this cPP. In particular, FIA_SOS.1 verifies secrets against a defined metric, but it does not express the requirement for an Administrator-settable minimum password length as part of the password management capability.

FIA_PMG_EXT.1 is therefore defined to state the intended password management requirement precisely as a single component: permitted password composition together with an Administrator-settable minimum password length. No existing Common Criteria Part 2 component clearly expresses this integrated password management capability as intended by this cPP.

The component is placed in the FIA class as a single component.
...
X.509 certificates are used by the SFRs for authentication, certificate validation, and certificate request generation, and the Common Criteria does not provide a suitable SFR for these functions.
 
This extended component ensures the validation, authentication, and request of X.509 certificates, and it is therefore placed in the FIA class.
...
This family addresses the requirements for verifying firmware/software integrity each time that it is powered on.
...
This extended component protects TSF data by preventing the reading of key material, and it is therefore placed in the FPT class.
...
FPT_TST_EXT.1 specifies a start-up operational self-test requirement: the TSF runs a suite of self-tests during initial start-up, including power-on, to demonstrate correct operation of the TSF.
 
The Common Criteria Part 2 component FPT_TST.1 also addresses TSF self-testing. However, FPT_TST.1 is defined as a three-element component. When claimed, it also requires verification of the integrity of TSF data and stored TSF executable code. These elements cannot be omitted. As a result, FPT_TST.1 cannot clearly express the intended cPP requirement without importing additional mandatory requirements that are outside the intended baseline.

Verification of stored TSF executable-code integrity is already addressed by this cPP through FPT_SBT_EXT.1, Secure Boot. FPT_SBT_EXT.1.2 requires boot-time verification of firmware/software integrity anchored in a hardware Root of Trust, which is specific to the boot architecture of HCD TOEs and more precise than the generic executable-code integrity verification required by FPT_TST.1.

This cPP establishes a security baseline that requires demonstration of correct TSF operation at start-up. It does not mandate general verification of TSF-data integrity across all conformant TOEs. Therefore, FPT_TST_EXT.1 is necessary to state the intended baseline requirement precisely, without importing FPT_TST.1 elements that are either already addressed by FPT_SBT_EXT.1 or outside the intended baseline. The extended component is placed in the FPT class as a single component.
...
|*U.NORMAL*
|Normal User
|A User who has been identified and authenticated and does not have an administrative role. A Normal User can be a Local User or a Network User as described in <<Operational Environment>>
...
[%header,cols="1,1"]
|===
|Examples of Protected TSF Data |Examples of Confidential TSF Data
...
|*O.ACCESS_CONTROL* restricts access to TSF Data in the TOE to authorized Users. +
*O.USER_I&A* provides the basis for access control. +
*O.ADMIN_ROLES* restricts the ability to authorize Users and set access controls to authorized Administrators.
...
|*P.KEY_MATERIAL* +
Cleartext keys, submasks, random numbers, or any other values that contribute to the creation of encryption keys for Nonvolatile Storage of User Document Data or Confidential TSF Data must be protected from unauthorized access and must not be stored on that storage device.
|*O.KEY_MATERIAL* protects keys and key materials from unauthorized access and ensures that any key materials are not stored in cleartext on the device that uses those materials for its own encryption.
...
| |FCS_COP.1/KeyEncap |This requirement defines the use of a strong method for key encapsulation. 
...
| |FCS_CKM.5 |This requirement defines the methods for generating an intermediary key.
...
| |FCS_SNI_EXT.1 |This requirement defines the creation and usage of salts and nonces.
...
| |FCS_CKM.5 |This requirement ensures the use of strong methods for performing cryptographic key derivation.
...
| FCS_CKM.1/SKG | [FCS_COP.1/DataEncryption, or FCS_COP.1/StorageEncryption, or FCS_COP.1/KeyWrap, or FCS_COP.1/KeyEnc, or FCS_COP.1/KeyedHash, or FCS_COP.1/CMAC], FCS_CKM.6, FCS_RBG_EXT.1 | FCS_COP.1/DataEncryption included, FCS_CKM.6 included, FCS_RBG_EXT.1 included, FCS_COP.1/StorageEncryption and FCS_COP.1/KeyWrap and FCS_COP.1/KeyEnc and FCS_COP.1/KeyedHash and FCS_COP.1/CMAC included as selection-based SFRs
| FCS_CKM.2 | FCS_CKM.1/AKG, FCS_CKM.6, [if selected: FCS_COP.1/KeyEncap] | FCS_CKM.1/AKG included, FCS_CKM.6 included, FCS_COP.1/KeyEncap included as selection-based SFR

| FCS_CKM.6 | FCS_CKM.1 or FCS_CKM.5 | FCS_CKM.1 included, or FCS_CKM.5 included as selection-based SFR
...
| FCS_COP.1/Hash | None | FCS_COP.1/Hash performs a keyless hashing operation; the key-source and key-destruction dependencies of FCS_COP.1 do not apply.
...
| FPT_SBT_EXT.1 | [if selected: FCS_COP.1/Hash], [if selected: FCS_COP.1/SigGen], [if selected: FCS_COP.1/KeyedHash and/or FCS_COP.1/CMAC]

| FCS_COP.1/Hash included if selected, FCS_COP.1/SigGen included if selected, FCS_COP.1/KeyedHash and/or FCS_COP.1/CMAC included if selected
...
| FPT_TUD_EXT.1
| [if selected: FCS_COP.1/SigGen], [if selected: FIA_X509_EXT.1], [if selected: FCS_COP.1/Hash]
| FCS_COP.1/SigGen included if selected, FIA_X509_EXT.1 included if selected, FCS_COP.1/Hash included if selected

| FTA_SSL.3 | FMT_SMR.1 | FMT_SMR.1 included
...
| FCS_COP.1/KeyEncap | FCS_CKM.1/AKG, FCS_CKM.6 | FCS_CKM.1/AKG included, FCS_CKM.6 included
...
| FCS_IPSEC_EXT.1 | [if selected: FIA_PSK_EXT.1], FCS_CKM.1/AKG, FCS_CKM.2, FCS_COP.1/DataEncryption, FCS_COP.1/SigGen, FCS_COP.1/Hash, [if selected: FCS_COP.1/KeyedHash], FCS_RBG_EXT.1 | FCS_CKM.1/AKG included, FCS_CKM.2 included, FCS_COP.1/DataEncryption included, FCS_COP.1/SigGen included, FCS_COP.1/Hash included, FCS_RBG_EXT.1 included, FIA_PSK_EXT.1 included if selected, FCS_COP.1/KeyedHash included if selected


| FCS_SSHC_EXT.1 | FCS_CKM.1/AKG, FCS_CKM.1/SKG, FCS_CKM.2, FCS_COP.1/DataEncryption, FCS_COP.1/SigGen, FCS_COP.1/Hash, FCS_COP.1/KeyedHash, FCS_RBG_EXT.1 | FCS_CKM.1/AKG included, FCS_CKM.1/SKG included, FCS_CKM.2 included, FCS_COP.1/DataEncryption included, FCS_COP.1/SigGen included, FCS_COP.1/Hash included, FCS_RBG_EXT.1 included, FCS_COP.1/KeyedHash included as selection-based SFRs



| FCS_SSHS_EXT.1 | FCS_CKM.1/AKG, FCS_CKM.2, FCS_COP.1/DataEncryption, FCS_COP.1/SigGen, FCS_COP.1/Hash, FCS_COP.1/KeyedHash, FCS_RBG_EXT.1 | FCS_CKM.1/AKG included, FCS_CKM.2 included, FCS_COP.1/DataEncryption included, FCS_COP.1/SigGen included, FCS_COP.1/Hash included, FCS_RBG_EXT.1 included, FCS_COP.1/KeyedHash included as selection-based SFR



| FCS_HTTPS_EXT.1 | FCS_TLSC_EXT, and/or FCS_TLSS_EXT | FCS_TLSC_EXT.1 and FCS_TLSS_EXT.1 included as selection-based SFRs

| FCS_COP.1/KeyedHash | FCS_RBG_EXT.1, FCS_COP.1/Hash, FTP_ITC.1 or FCS_CKM.1/AKG, FCS_CKM.6 | FCS_RBG_EXT.1 included, FCS_COP.1/Hash included, FTP_ITC.1 and FCS_CKM.1/AKG included, FCS_CKM.6 included

| FIA_PSK_EXT.1
| [if selected: FCS_COP.1/Hash], [if selected: FCS_RBG_EXT.1]
| FCS_COP.1/Hash included if selected, FCS_RBG_EXT.1 included if selected


| FCS_PCC_EXT.1 | FCS_COP.1/KeyedHash | FCS_COP.1/KeyedHash included as selection-based SFR

| FCS_CKM.5 | [if selected: FCS_COP.1/KeyedHash], [if selected: FCS_COP.1/CMAC], FCS_CKM.6, [if selected: FCS_RBG_EXT.1] |FCS_COP.1/KeyedHash included if selected, FCS_COP.1/CMAC included if selected, FCS_CKM.6 included, FCS_RBG_EXT.1 included if selected

| FCS_COP.1/CMAC | FCS_CKM.1/SKG, FCS_CKM.6 | FCS_CKM.1/SKG included, FCS_CKM.6 included.
...
| FIA_X509_EXT.1 | FIA_X509_EXT.2 | FIA_X509_EXT.2 included as selection-based SFR

| FIA_X509_EXT.2 | FIA_X509_EXT.1 | FIA_X509_EXT.1 included as selection-based SFR

| FIA_X509_EXT.3 | FCS_CKM.1/AKG, FIA_X509_EXT.1 | FCS_CKM.1/AKG included, FIA_X509_EXT.1 included as selection-based SFR
...
| FPT_KYP_EXT.1 | FCS_KYC_EXT.1, [if selected: FCS_COP.1/KeyWrap], [if selected: FCS_COP.1/KeyEnc and/or FCS_COP.1/KeyTransport], [if selected: FCS_SMC_EXT.1] | FCS_KYC_EXT.1 included as conditionally-mandatory SFR, FCS_COP.1/KeyWrap included if selected, FCS_COP.1/KeyEnc included if selected, FCS_COP.1/KeyTransport included if selected, FCS_SMC_EXT.1 included if selected
...
| FCS_KYC_EXT.1 | FCS_COP.1/KeyWrap, FCS_SMC_EXT.1, FCS_CKM.6, FCS_COP.1/KeyEnc, FCS_CKM.5, and/or FCS_COP.1/KeyTransport | FCS_CKM.6 included, FCS_COP.1/KeyWrap and FCS_SMC_EXT.1 and FCS_COP.1/KeyEnc and FCS_CKM.5 and FCS_COP.1/KeyTransport included as selection-based SFRs
