...
| 1.1e | 10 September 2026 | Errata version
...
* [#SD]#[SD]# Supporting Document Mandatory Technical Document: Evaluation Activities for collaborative Protection Profile for Hardcopy Devices, v1.1e, 10 September 2026.
...
==== Firmware Integrity
The TOE verifies the integrity of its firmware/software [O.FW_INTEGRITY] during the boot process.
...
|O.FW_INTEGRITY
|P.ROT_INTEGRITY
|The TOE verifies the integrity of its firmware/software during the boot process.
...
*FAU_GEN.1.1 Refinement* The TSF shall be able to generate audit data of the following auditable events:

..	Start-up and shutdown of the audit functions;
..	All auditable events for the [not specified] level of audit; and
..	*All auditable events specified in Table 3*, [assignment: _other specifically defined auditable events_].

*FAU_GEN.1.2 Refinement* The TSF shall record within each audit data at least the following information:
...
*FCS_COP.1.1/SigGen Refinement:* The TSF shall perform [*cryptographic signature services (generation and verification)*] in accordance with a specified cryptographic algorithm [selection:
...
*_+++<del>and cryptographic key sizes [assignment: cryptographic key sizes]</del>+++_* that meet the following: [selection:
...
| [assignment: list of TSF Data owned by a U.NORMAL or associated with Documents or jobs owned by a U.NORMAL] | [selection: change default, query, modify, delete, clear, [assignment: other operations]] | U.ADMIN, the{nbsp}owning{nbsp}U.NORMAL.
...
[if selected: FCS_COP.1/DataEncryption Cryptographic Operation (Data Encryption/Decryption), and/or FCS_COP.1/StorageEncryption Cryptographic Operation (Data Encryption/Decryption)] + 
...
*_Refinement Rationale:_*

The term “trusted communication channel” is equivalent to the communication channel required by <<CC2>> FTP_ITC.1.1. The refined element retains all properties required of the communication channel by FTP_ITC.1.1, including logical distinction from other communication channels, assured identification of its end points, and protection of channel data from modification or disclosure. The qualifier “trusted” reflects the terminology used by <<CC2>> for the FTP_ITC family and does not remove or weaken any requirement of the original component.

*_Application Note:_*

_For purposes of FTP_ITC.1, an “authorized IT entity” is a trusted IT product, in the sense used by <<CC2>>, that is configured in the evaluated configuration as an authorized endpoint for one or more of the capabilities identified in FTP_ITC.1.1._
...
* _[ISO/IEC 18033-3 (AES), [selection: NIST SP 800-38F, ISO/IEC 19772, no other standards]]_,
...
] [.line-through]#*_and cryptographic key sizes [assignment: cryptographic key sizes] that meet the following: [assignment: list of standards]_*#.
...
_If TLS is selected in FTP_ITC, then certificate validity is tested in accordance with testing performed for FIA_X509_EXT.1._
...
_If DTLS is selected in FTP_ITC then certificate validity is tested in accordance with testing performed for FIA_X509_EXT.1._
...
*FCS_CKM.5.1* The TSF shall derive cryptographic keys [*intermediate key*] from [selection: _a RNG generated submask as specified in FCS_RBG_EXT.1, a conditioned password submask, imported submask_] in accordance with a specified key derivation algorithm [*the keyed-hash functions specified in FCS_COP.1/CMAC*] and specified cryptographic key sizes [*at least of equivalent security strength (in number of bits) to the BEV or the DEK*] that meet the following: [selection: _NIST SP 800-108 [selection: KDF in Counter Mode, KDF in Feedback Mode, KDF in Double-Pipeline Iteration Mode], NIST SP 800-132, ISO/IEC 11770-6:2016 [selection: KPF2, KPF3, KPF4]_].
...
_This iteration supports both cipher-based message authentication (CMAC) and keyed-hash message authentication (HMAC) to serve the message-authentication needs of the SFRs that depend on it. Although the iteration is identified as “for cipher-based message authentication”, the HMAC algorithms are included to provide a keyed-hash MAC option where an SFR requires one._
...
X.509 certificate-based authentication is required if IPsec or TLS communications are claimed for FTP_ITC.1 or FTP_TRP. X.509 certificate validation is also required if "X.509 Certificate" is selected for trusted update in FPT_TUD_EXT.1.3. If SSH client communications are claimed and any x509 algorithms are claimed in FCS_SSHC_EXT.1.5 or FCS_SSHS_EXT.1.5, these SFRs are required. In the case of the TOE only acting as the SSH server or acting as the client, but not claiming any x509 algorithms in FCS_SSHC_EXT.1.5 or FCS_SSHS_EXT.1.5, these SFRs are optional.
...
*FTP_TRP.1.2/NonAdmin* The TSF shall permit [selection: _the TSF, remote users_] to initiate communication via the trusted path.

*FTP_TRP.1.3/NonAdmin* The TSF shall require the use of the trusted path for [initial user authentication *[and all remote user actions]*].
...
Common Criteria Part 2 includes a random bit generation component, FCS_RBG.1, which is the closest existing component to FCS_RBG_EXT.1. FCS_RBG.1 carries a defined dependency structure: dependencies on FCS_RBG.2 or FCS_RBG.3 for external or internal seeding, and on FPT_FLS.1, Failure with preservation of secure state, and FPT_TST.1, TSF self-testing. Under CC2022-P2-R1-0035, applied to CC:2022 through CC Errata and Interpretations v1.2, these dependencies shall always be met, and the CC Part 1 mechanism for justifying an unsatisfied dependency is not permitted for this component. Use of FCS_RBG.1 therefore requires the dependencies on FPT_FLS.1 and FPT_TST.1 to be satisfied.

FPT_FLS.1 requires preservation of a secure state upon identified TSF failures, and FPT_TST.1 requires, in addition to self-testing, integrity-verification capabilities provided to authorized users (FPT_TST.1.2 and FPT_TST.1.3). This introduces security functionality beyond the random bit generation baseline established for conformant TOEs, which is limited to the deterministic random bit generation mechanism, the applicable standard, and the entropy-source seeding requirement. FCS_RBG_EXT.1 states this baseline precisely as a single, self-contained component. It is not possible to clearly express this functionality using the existing Part 2 components without also incorporating the additional failure-state and self-testing functionality imposed by the mandatory dependencies of FCS_RBG.1.
...
TLS is used by the SFRs to establish a trusted channel/path as a client, and the Common Criteria does not provide a suitable SFR for the TLS client protocol.

This extended component ensures the secure operation of the TLS client protocol, and it is therefore placed in the FCS class.
...
TLS is used by the SFRs to establish a trusted channel/path as a server, and the Common Criteria does not provide a suitable SFR for the TLS server protocol.

This extended component ensures the secure operation of the TLS server protocol, and it is therefore placed in the FCS class.
...
DTLS is used by the SFRs to establish a trusted channel/path, and the Common Criteria does not provide a suitable SFR for the DTLS protocol.

This extended component ensures the secure operation of the DTLS protocol, and it is therefore placed in the FCS class.
...
DTLS is used by the SFRs to establish a trusted channel/path as a client, and the Common Criteria does not provide a suitable SFR for the DTLS client protocol.

This extended component ensures the secure operation of the DTLS client protocol, and it is therefore placed in the FCS class.
...
DTLS is used by the SFRs to establish a trusted channel/path as a server, and the Common Criteria does not provide a suitable SFR for the DTLS server protocol.

This extended component ensures the secure operation of the DTLS server protocol, and it is therefore placed in the FCS class.
...
The Common Criteria Part 2 component FPT_TST.1 also addresses TSF self-testing and is the closest existing component to FPT_TST_EXT.1. FPT_TST.1 is defined as a three-element component, and the complete set of elements must be selected for inclusion when the component is claimed. In addition to the self-test requirement of FPT_TST.1.1, FPT_TST.1.2 and FPT_TST.1.3 require the TSF to provide authorized users with the capability to verify the integrity of TSF data, or parts of TSF data, and of the TSF, or parts of the TSF. The selections in these elements permit the objects of verification to be narrowed, but do not permit the authorized-user verification capability itself to be removed, and a refinement that removed that capability would not be a valid refinement of the component.

This cPP establishes a security baseline that requires demonstration of correct TSF operation at start-up. It does not require the TSF to provide authorized users with integrity-verification capabilities for TSF data or for the TSF. Claiming FPT_TST.1 would therefore introduce mandatory security functionality beyond this baseline at any permitted completion of its elements. It is not possible to clearly express the start-up self-test requirement using the existing Part 2 components and their available operations without also incorporating that additional functionality.

FPT_TST_EXT.1 is therefore retained and placed in the FPT class as a single component.
...
Finisher:: 
A hardware device, integrated into or attached to an HCD, that performs post-processing operations on printed output, such as stapling, hole punching, folding, collating, or booklet making.
...
|*P.ROT_INTEGRITY* +
The vendor provides a Root of Trust (RoT) that is comprised of the TOE firmware, hardware, and pre-installed public keys or required critical security parameters.
|*O.FW_INTEGRITY* ensures that the TOE verifies the integrity of its firmware/software during the boot process.
...
| FPT_SBT_EXT.1
| [if selected: FCS_COP.1/Hash], [if selected: FCS_COP.1/SigGen], [if selected: FCS_COP.1/KeyedHash and/or FCS_COP.1/CMAC], [if selected: FCS_COP.1/DataEncryption and/or FCS_COP.1/StorageEncryption]
| FCS_COP.1/Hash included if selected, FCS_COP.1/SigGen included if selected, FCS_COP.1/KeyedHash and/or FCS_COP.1/CMAC included if selected, FCS_COP.1/DataEncryption and/or FCS_COP.1/StorageEncryption included if selected
...
| FCS_CKM.5 | FCS_COP.1/CMAC, FCS_CKM.6, [if selected: FCS_RBG_EXT.1] | FCS_COP.1/CMAC included, FCS_CKM.6 included, FCS_RBG_EXT.1 included if selected

| FCS_COP.1/CMAC | FCS_CKM.1/SKG, [if selected: FCS_COP.1/Hash], FCS_CKM.6 | FCS_CKM.1/SKG included, FCS_COP.1/Hash included if selected, FCS_CKM.6 included
...
| FCS_KYC_EXT.1, [if selected: FCS_COP.1/KeyWrap], [if selected: FCS_COP.1/KeyEnc and/or FCS_COP.1/KeyTransport], [if selected: FCS_SMC_EXT.1]
| FCS_KYC_EXT.1 included as conditionally-mandatory SFR, FCS_COP.1/KeyWrap included if selected, FCS_COP.1/KeyEnc and/or FCS_COP.1/KeyTransport included if selected, FCS_SMC_EXT.1 included if selected
