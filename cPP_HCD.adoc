...
| 2.0-pre.1 | 10 May 2026 | Working Draft v2.0
...
• CCM: Nonce shall be non-repeating, unpredicatable, and at least 64 bits in size.
...
• GCM: The IVs are unique values (hence non-repeating) with lengths in the range 1 &lt;= len(IV) &lt;= 2^64-1. Additionally, the number of invocations of GCM shall not exceed 2^32 for a given secret key (unless the IV length is 96 and a deterministic construction is used).
