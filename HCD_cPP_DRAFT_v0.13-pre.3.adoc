...
| 0.13-pre.3 | 25 July 2022 | Editor's Review
...
*FDP_UDU_EXT.1.1 Extended:* The TSF shall ensure that any previous information content stored on a [selection: *wear-leveled storage device*, *non-wear-leveled storage device*] of a resource is made unavailable [selection: *by overwriting data*, *by destroying its cryptographic key*] upon the *deallocation of the resource from* the following objects: *D.USER.DOC*.
