...
This family defines requirements for the TSF to ensure that secure transmission of audit
...
*FAU_STG_EXT.1* External Audit Trail Storage requires the TSF to use a trusted
...
The TSF is required that the transmission of generated audit data to an External IT Entity
...
This family addresses the management aspects of cryptographic keys. Especially, this
...
*FCS_CKM_EXT.4* Cryptographic Key Material Destruction ensures not only keys but
...
Cryptographic Key Material Destruction is to ensure the keys and key materials that are
...
Components in this family define requirements for protecting remote management
...
*FCS_HTTPS_EXT.1* HTTPS selected, requires that HTTPS be implemented according
...
HTTPS is one of the secure communication protocols, and the Common Criteria does not
...
FCS_IPSEC_EXT.1.2 The TSF shall implement [selection: tunnel mode, transport
mode].
...
by RFC 3602) together with a Secure Hash Algorithm (SHA)-based HMAC, AES-CBC256 (as specified by RFC 3602) together with a Secure Hash Algorithm (SHA)-based
HMAC, AES-GCM-128 as specified in RFC 4106, AES-GCM-256 as specified in RFC
4106].

FCS_IPSEC_EXT.1.5 The TSF shall implement the protocol: [selection: IKEv1 as
...
AES-CBC-256 as specified in RFC 3602 and [selection: AES-GCM-128, AES-GCM-256
as specified in RFC 5282, no other algorithm].

FCS_IPSEC_EXT.1.7 The TSF shall ensure that IKEv1 Phase 1 exchanges use
only main mode.

FCS_IPSEC_EXT.1.8 The TSF shall ensure that [selection: IKEv2 SA lifetimes
...
IPsec is one of the secure communication protocols, and the Common Criteria does not
provide a suitable SFR for the communication protocols using cryptographic algorithms.
...
This family specifies the means by which an intermediate key is derived from a specified
set of submasks.

Component leveling

|===
FCS_KDF_EXT Cryptograhic Key Derivation |  1
|===
...
authentication), [if selected: FCS_RBG_EXT.1 Extended: Cryptographic Operation
(Random Bit Generation)]

FCS_KDF_EXT.1.1 The TSF shall accept [selection: a RNG generated submask as
...
This family provides the specification to be used for using multiple layers of encryption
...
FCS_KYC_EXT Key Chaining, requires the TSF to maintain a key chain and specifies
...
Key Chaining ensures that the TSF maintains the key chain, and also specifies the
...
FCS_PCC_EXT.1 Extended: Cryptographic Password Construction and
Conditioning
...
accordance with a specified cryptographic algorithm [HMAC-[selection: SHA-256, SHA384, SHA-512]], with [assignment: positive integer of 1000 or more] iterations, and
...
A.9.8 FCS_RBG_EXT Extended: Cryptographic Operation (Random Bit
Generation)

Family Behavior:

This family defines requirements for random bit generation to ensure that it is performed
...
Random bits/number will be used by the SFRs for key generation and destruction, and
...
This family defines the means by which submasks are combined, if the TOE supports
...
Submask Combining is to ensure the TSF combine the submasks in order to derive or
protect the BEV.
...
This family addresses the ability for a server and/or a client to offer SSH to protect data
...
FCS_SSH_EXT.1.1 The TSF shall implement the SSH protocol that complies with
...
supports the following authentication methods as described in RFC 4252: public keybased, password-based.

FCS_SSH_EXT.1.3 The TSF shall ensure that, as described in RFC 4253, packets
greater than [assignment: number of bytes] bytes in an SSH transport connection are
dropped.

FCS_SSH_EXT.1.4 The TSF shall ensure that the SSH transport implementation uses
...
[selection: SSH_RSA, ecdsa-sha2-nistp256] and [selection: PGP-SIGN-RSA, PGP-SIGNDSS, ecdsa-sha2-nistp384, no other public key algorithms,] as its public key algorithm(s).
...
FCS_SSH_EXT.1.7 The TSF shall ensure that diffie-hellman-group14-sha1 and
...
SSH is one of the secure communication protocols, and the Common Criteria does not
provide a suitable SFR for the communication protocols using cryptographic algorithms.
...
This family addresses the ability for a server and/or a client to use TLS to protect data
...
FCS_TLS_EXT.1 TLS selected, requires the TLS protocol implemented as
specified.
...
TLS is one of the secure communication protocols, and the Common Criteria does not
provide a suitable SFR for the communication protocols using cryptographic algorithms.
...
FDP_DSK_EXT.1 Extended: Protection of Data on Disk, requires the TSF to encrypt all
...
is separately CC certified to conform to the FDE EE cPP] such that any FieldReplaceable Nonvolatile Storage Device contains no plaintext User Document Data and
...
Extended: Protection of Data on Disk is to specify that encryption of any confidential
...
This family addresses the requirements for separation between Fax PSTN line and the
...
FDP_FXS_EXT.1 Fax Separation, requires the fax interface cannot be used to create a
...
Fax Separation is to protect a LAN against attack from PSTN line, and the Common
Criteria does not provide a suitable SFR for the Protection of TSF or User Data.

This extended component protects the TSF Data or User Data, and it is therefore placed
...
This family defines requirements for the attributes of passwords used by administrative
...
FIA_PMG _EXT.1 Password management requires the TSF to support passwords with
...
FIA_PMG _EXT.1.1The TSF shall provide the following password management
capabilities for User passwords:
...
This family defines requirements for the TSF to ensure the ability to use pre-shared keys
for IPsec.
...
FIA_PSK_EXT.1 Pre-Shared Key Composition, ensures authenticity and access control
for updates.
...
FIA_PSK_EXT.1.3 The TSF shall condition the text-based pre-shared keys by using [selection:
...
Pre-shared Key Composition is to ensure the strong authentication between the endpoints
of communications, and the Common Criteria does not provide a suitable SFR for the
Pre-shared Key Composition.

This extended component protects the TOE by means of strong authentication, and it is
therefore placed in the FIA class with a single component.
...
This family addresses the requirements for keys and key materials to be protected if and
...
FPT_KYP_EXT.1 Extended: Protection of key and key material, requires the TSF to
...
FPT_KYP_EXT.1.1 The TSF shall not store plaintext keys that are part of the
...
Protection of Key and Key Material is to ensure that no plaintext key or key material are
...
This family addresses the requirements for managing and protecting the TSF data, such
...
Protection of TSF Data is to ensure the pre-shared keys, symmetric keys and private keys
...
FPT_TST_EXT.1 TSF testing requires a suite of self-testing to be run during initial
...
TSF testing is to ensure the TSF can be operated correctly, and the Common Criteria does
...
This family defines requirements for the TSF to ensure that only administrators can
...
generation/verification), or FCS_COP.1(c) Cryptographic operation (Hash Algorithm)].
...
Firmware/software is a form of TSF Data, and the Common Criteria does not provide a
