...
| 0.13-pre | 17 July 2022 | Public Draft Final
...
|*O.IMAGE_OVERWRITE* (optional) |FPT_UDU_EXT.1	|This requirement enforces the overwriting of user document data stored on the TOE after each job is processed or cancelled.
...
==== FPT_UDU_EXT.1 USER.DOC Unavailability
======
...
*FPT_UDU_EXT.1.1 Extended:* The TSF shall ensure that any previous information content stored on a [selection: *wear-leveled storage device*, *non-wear-leveled storage device*] of a resource is made unavailable [selection: *by overwriting data*, *by destroying its cryptographic key*] upon the *deallocation of the resource from* the following objects: *D.USER.DOC*.
...
.Security Functional Requirements
|===
|Objective/SFR |O.ACCESS_ CONTROL |O.ADMIN_ ROLES |O.AUDIT |O.COMMS_ PROTECTION |O.FAX_ NET_ SEPARATION |O.IMAGE_ OVERWRITE |O.KEY_ MATERIAL |O.PURGE_ DATA |O.STORAGE_ ENCRYPTION |O.TSF_ SELF_ TEST |O.UPDATE_ VERIFICATION |O.USER_ AUTHORIZATION |O.USER_ I&A |O.AUTH_ FAILURE |O.FW_ INTEGRITY |O.STRONG_ CRYPTO
...
The TOE shall provide a function that an authorized administrator may destroy encryption keys or keying material if the TOE supports a function for removing the TOE from its Operational Environment.
