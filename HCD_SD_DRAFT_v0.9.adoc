...
protocol selected in FTP_TRP.1(a), FTP_TRP.1(b) and FTP_ITC.1 that uses RSAES-PKCS1-v1_5.

*_Diffie-Hellman Group 14_*

The evaluator shall verify the correctness of the TSF’s implementation of
Diffie-Hellman group 14 by using a known good implementation for each
protocol selected in FTP_TRP.1(a), FTP_TRP.1(b) and FTP_ITC.1 that uses Diffie-Hellman group 14.
...
selected in FTP_TRP.1(a), FTP_TRP.1(b) and FTP_ITC.1 that uses safe-prime groups. This test must be performed for each safe-prime
...
in FCS_COP.1/DataEncryption, the AES-CBC Known Answer Test, AESGCM
...
or calculated min-entropy supplied either separately by each source or the minentropy
...
to the Output Block Length (as defined in SP800-90A).
...
The evaluator shall verify the TSS includes a description of the key derivation function and shall verify the key derivation uses an approved derivation mode and key expansion algorithm according to SP800-108 and SP800-132.
...
.. Test 8: (Conditional on support for EC certificates as indicated in FCS_COP.1(b)). The evaluator shall conduct the following tests:
...
The evaluator shall repeat these tests for each distinct use of certificates. Thus, for example, use of certificates for TLS connection is distinct from use of certificates for trusted updates so both of these uses would be tested. But there is no need to repeat the tests for each separate TLS channel/path in FTP_ITC.1,  FTP_TRP.1(a) and FPT_TRP.1(b) (unless the channels/paths use separate implementations of TLS). 
