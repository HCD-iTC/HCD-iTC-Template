...
The evaluator shall include test cases of FCS_CKM.5 to the test subset. Note that FCS_CKM.5 may be not mapped to the specific interface(s) after evaluator's analysis during ADV_FSP.1.
...
. One or more pseudorandom functions (PRFs) that are included in the 'cryptographic key derivation algorithm' selection in the SFR, and their output lengths in bits (h)
...
. the 'cryptographic key sizes' selections in the SFR, i.e the lengths (in bits) of the derived keying material (L)
