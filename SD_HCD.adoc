...
| 2.0-pre.2  | 8 June 2026 | Draft 2.0
|===
...
* latexmath:[\operatorname{GCD}(p-1, e) = 1]
* latexmath:[\operatorname{GCD}(q-1, e) = 1]
* latexmath:[|p - q| > 2^{(nlen/2) - 100}]
* latexmath:[p \geq \sqrt{2} \cdot 2^{(nlen/2) - 1}]
* latexmath:[q \geq \sqrt{2} \cdot 2^{(nlen/2) - 1}]
* latexmath:[2^{nlen/2} < d < \operatorname{LCM}(p-1, q-1)]
* latexmath:[e \cdot d = 1 \bmod \operatorname{LCM}(p-1, q-1)]
...
* stem:[\operatorname{len}(q)]-bit output of the RBG, where stem:[1 \le x \le q-1]
* stem:[\operatorname{len}(q) + 64]-bit output of the RBG, followed by a stem:[\bmod (q-1)] operation and a stem:[+1] operation, where stem:[1 \le x \le q-1]
...
* stem:[\operatorname{len}(q)]-bit output of the RBG, where stem:[2^{\operatorname{len}(q)-1} < x < q]
* stem:[\operatorname{len}(q) + 64]-bit output of the RBG, followed by a stem:[\bmod (q-1)] operation and a stem:[+1] operation, where stem:[0 < x < q]
...
. stem:[2^{\operatorname{len}(p)-1} < p < 2^{\operatorname{len}(p)}]
. stem:[2^{\operatorname{len}(q)-1} < q < 2^{\operatorname{len}(q)}]
. stem:[q \mid (p-1)]
. stem:[1 < g < p]
. stem:[g^q \bmod p = 1]
. stem:[0 < x < q]
...
Generate one ciphertext using algorithm KWP-AE with substring stem:[[\operatorname{len}(P)/8\]_{32}] of S replaced by each of the following 32-bit values, where stem:[\operatorname{len}(P)] is the length of P in bits and stem:[[\,\]_{32}] denotes representation of an integer in 32 bits:

. stem:[[0\]_{32}]
. stem:[[\operatorname{len}(P)/8-8\]_{32}]
. stem:[[\operatorname{len}(P)/8+8\]_{32}]
. stem:[[513\]_{32}]
...
==== FCS_SNI_EXT.1 Cryptographic Operation (Salt and Nonce)
