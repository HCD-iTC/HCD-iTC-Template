...
. If KCDSA is selected, the evaluator shall verify that the TSS specifies the methods used to generate p, q and g, and
...
verification exponent stem:[e], the private prime factors stem:[p] and stem:[q], the public modulus
stem:[n] and the calculation of the private signature exponent stem:[d].

Key Pair generation specifies 5 ways (or methods) to generate the primes stem:[p]
and stem:[q]. These include:
...
* Primes stem:[p1], stem:[p2], stem:[q1], stem:[q2], stem:[p] and stem:[q] shall all be provable primes
* Primes stem:[p1], stem:[p2], stem:[q1], and stem:[q2] shall be provable primes and stem:[p] and stem:[q]
shall be probable primes
* Primes stem:[p1], stem:[p2], stem:[q1], stem:[q2], stem:[p] and stem:[q] shall all be probable primes
...
The evaluator shall verify the implementation of Parameter Generation and Key
Generation for FFC by the TOE using the Parameter Generation and Key Generation
test. This test verifies the ability of the TSF to correctly produce values for the
field prime stem:[p], the cryptographic prime stem:[q] (dividing stem:[p-1]),
the cryptographic group generator stem:[g], and the calculation of the private
key stem:[x] and public key stem:[y].
...
The key generation specifies 2 ways to generate the private key stem:[x]:

* stem:[len(q)]-bit output of the RBG where stem:[1 <= x <= q-1]
* stem:[len(q) + 64]-bit output of the RBG, followed by a mod stem:[q-1] operation and a stem:[+1] operation, where stem:[1 <= x <= q-1]
...
* len(q) bit output of RBG where 2^(len(q)-1)<x<q
* len(q) + 64 bit output of RBG, followed by a mod q-1 operation and +1 operation where 0< x<q
...
....
1. 2^(len(p)-1)<p<2^(len(p))
2. 2^(len(q)-1)<q<2^(len(q))
3. q divides p-1
4. 1<g<p 
5. g^q mod p = 1 
6. 0<x<q 
7. g^x' mod p = y, where x' = x^-1 mod q
....
...
values for each selected key size as described below and obtain the ciphertext
...
result from encryption of the given plaintext using each selected key size with a key value of all zeros (e.g., 256 ciphertext values will be generated if 128 bits
and 256 bits are selected and 384 ciphertext values will be generated if all
key sizes are selected). Plaintext value i in each set shall have the leftmost bits
...
Key[0] = KEY, IV[0] = IV, PT[0] = PT
   For i = 0 to 99 {
       ...
       For j = 0 to 999 {
           If ( j=0 ) {
               CT[j] = HIGHT_CBC_ENCRYPT(Key[i], IV[i], PT[j])
               PT[j+1] = IV[i]
           } else {
               CT[j] = HIGHT_CBC_ENCRYPT(Key[i], PT[j])
               PT[j+1] = CT[j-1]
           }
       }
       Output CT[j]
...
Key[0] = KEY, IV[0] = IV, PT[0] = PT
   For i = 0 to 99 {
       ...
       For j = 0 to 999 {
           If ( j=0 ) {
               CT[j] = HIGHT_CFB_ENCRYPT(Key[i], IV[i], PT[j])
               PT[j+1] = IV[i]
           } else {
               CT[j] = HIGHT_CFB_ENCRYPT(Key[i], PT[j])
               PT[j+1] = CT[j-1]
           }
       }
       Output CT[j]
...
Key[0] = KEY, IV[0] = IV, PT[0] = PT
   For i = 0 to 99 {
       ...
       For j = 0 to 999 {
           If ( j=0 ) {
               OT[j] = HIGHT_ENCRYPT(Key[i], IV[i])
               CT[j] = PT[j] XOR OT[j]
               PT[j+1] = IV[i]
           } else {
               OT[j] = HIGHT_ENCRYPT(Key[i], OT[j-1])
               CT[j] = PT[j] XOR OT[j]
               PT[j+1] = CT[j-1]
           }
       }
...
....
   ...
   For i = 0 to 99 {
       Output Key[i], CTR[0], PT[0]
       For j = 0 to 999 {
           CT[j] = PT[j] XOR HIGHT_ENCRYPT(Key[i], CTR[0])
           CTR[0] = (CTR[0] + 1) MOD 2^128
           PT[j+1] = CT[j]
       }
       Output CT[j]
...
Key[0] = KEY, IV[0] = IV, PT[0] = PT
   For i = 0 to 99 {
       ...
       For j = 0 to 999 {
           If ( j=0 ) {
               CT[j] = LEA_CFB_ENCRYPT(Key[i], IV[i], PT[j])
               PT[j+1] = IV[i]
           } else {
               CT[j] = LEA_CFB_ENCRYPT(Key[i], PT[j])
               PT[j+1] = CT[j-1]
           }
       }
       Output CT[j]
...
Key[0] = KEY, IV[0] = IV, PT[0] = PT
   For i = 0 to 99 {
       ...
       For j = 0 to 999 {
           If ( j=0 ) {
               OT[j] = LEA_ENCRYPT(Key[i], IV[i])
               CT[j] = PT[j] XOR OT[j]
               PT[j+1] = IV[i]
           } else {
               OT[j] = LEA_ENCRYPT(Key[i], OT[j-1])
               CT[j] = PT[j] XOR OT[j]
               PT[j+1] = CT[j-1]
           }
       }
...
Key[0] = KEY, CTR[0] = CTR, PT[0] = PT
   For i = 0 to 99 {
       Output Key[i], CTR[0], PT[0]
       For j = 0 to 999 {
           CT[j] = PT[j] XOR LEA_ENCRYPT(Key[i], CTR[0])
           CTR[0] = (CTR[0] + 1) MOD 2^128
           PT[j+1] = CT[j]
       }
       Output CT[j]
...
For each domain parameter (i.e., (p=2048, q=224), (p=2048, q=256)) and SHA function pair, the evaluator shall generate a set of 10 1024-bit messages. The test passes only if all the signatures with submitted parameters result in successful signature verification.
...
1.	Ciphertext with length greater than 65 semi-blocks
2.	Ciphertext with bit-length not divisible by 64
3.	Ciphertext with length 0, 4) ciphertext with length of one semi-block.
...
For each supported combination of PRF, counter location (if a counter is used), value of r (if a counter is used), value of L, and IV length, the evaluator shall generate 20 pseudorandom key derivation key values (K~I~).
