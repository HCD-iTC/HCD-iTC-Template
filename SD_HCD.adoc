...
:pp-shortname: HCDcPP
:pp-version: 2.0-pre.4
:pp-date: 30 July 2026
...
Hardcopy Devices international Technical Community (HCD-iTC)
...
| 2.0-pre.1  | 10 May 2026 | Draft 2.0
| 2.0-pre.2  | 8 June 2026 | Draft 2.0 #2
| 2.0-pre.3  | 20 July 2026 | Draft 2.0 #3
| 2.0-pre.4  | 30 July 2026 | Draft 2.0 #4
|===
...
If the TOE uses the generated key in a key chain/hierarchy, then the evaluator shall examine the TOE summary specification to confirm that it describes the following:

. If KCDSA is selected, the evaluator shall verify that the TSS specifies the methods used to generate stem:[p], stem:[q] and stem:[g].
...
The evaluator shall include test cases of FCS_CKM.1/AKG to the test subset. Note that FCS_CKM.1/AKG may not be mapped to the specific interface(s) after evaluator’s analysis during ADV_FSP.1.
...
FIPS PUB 186-5 Key Pair generation specifies five methods for generating the primes stem:[p] and stem:[q]. These are:
...
* Hash algorithm [SHA-384, SHA-512] (methods 1, 3, and 4 only)
* Miller-Rabin primality test with error probability [latexmath:[2^{-100}] or latexmath:[2^{-\text{Security Strength}}]] (methods 2, 4, and 5 only)
* latexmath:[p \bmod 8] value [1, 3, 5, or 7]
* latexmath:[q \bmod 8] value [1, 3, 5, or 7]
* Private key format [standard, Chinese Remainder Theorem]
* Public exponent [fixed value, random]

The evaluator shall verify the ability of the TSF to correctly produce values for the RSA key components, including the public verification exponent stem:[e], the private prime factors stem:[p] and stem:[q], the public modulus stem:[n], and the calculation of the private signature exponent stem:[d].

*_Testing for Random Provable Primes and Conditional Methods_*
...
*_Testing for Random Probable Primes Method_*
...
* latexmath:[n = p \cdot q]
* latexmath:[p] and latexmath:[q] are probably prime according to Miller-Rabin tests with error probability latexmath:[< 2^{-125}]
* latexmath:[2^{16} < e < 2^{256}] and latexmath:[e] is an odd integer
* latexmath:[\mathrm{GCD}(p-1, e) = 1]
* latexmath:[\mathrm{GCD}(q-1, e) = 1]
* latexmath:[|p - q| > 2^{(nlen/2) - 100}]
* latexmath:[p \geq \sqrt{2} \cdot 2^{(nlen/2) - 1}]
* latexmath:[q \geq \sqrt{2} \cdot 2^{(nlen/2) - 1}]
* latexmath:[2^{nlen/2} < d < \mathrm{LCM}(p-1, q-1)]
* latexmath:[e \cdot d = 1 \bmod \mathrm{LCM}(p-1, q-1)]
...
For each supported combination of the above input parameters the evaluator shall require the implementation under test to generate 10 private/public key pairs latexmath:[(d, Q)]. The private key, latexmath:[d], shall be generated using a random bit generator as specified in FCS_RBG.1. The private key, latexmath:[d], is used to compute the public key, latexmath:[Q']. The evaluator shall confirm that latexmath:[0 < d < n] (where latexmath:[n] is the order of the group), and the computed value latexmath:[Q'] is then compared to the generated public/private key pairs' public key, latexmath:[Q], to confirm that latexmath:[Q] is equal to latexmath:[Q'].

*_ECC Key Validation Test_*

For each supported combination of the above parameters the evaluator shall generate 12 private/public key pairs using the key generation function of a known-good implementation. For each set of 12 public keys, the evaluator shall modify four public key values by shifting latexmath:[x] or latexmath:[y] out of range by adding the order of the field and modify four other public key values by shifting latexmath:[x] or latexmath:[y] so that they are still in bounds, but not on the curve. The remaining public key values are left unchanged (i.e., correct). To determine correctness, the evaluator shall submit the public keys to the public key validation (PKV) function of the TOE and shall confirm that the results correspond as expected for the modified and unmodified values.

*Key Generation for Finite-Field Cryptography (FFC)*

The evaluator shall verify the implementation of Parameter Generation and Key Generation for FFC by the TOE using the Parameter Generation and Key Generation test. This test verifies the ability of the TSF to correctly produce values for the field prime stem:[p], the cryptographic prime stem:[q] (dividing stem:[p-1]), the cryptographic group generator stem:[g], and the calculation of the private key stem:[x] and public key stem:[y].

The Parameter generation specifies 2 ways (or methods) to generate the
cryptographic prime stem:[q] and the field prime stem:[p]:


* Primes stem:[q] and stem:[p] shall both be provable primes
* Primes stem:[q] and field prime stem:[p] shall both be probable primes

and two ways to generate the cryptographic group generator stem:[g]:

* Generator stem:[g] constructed through a verifiable process
* Generator stem:[g] constructed through an unverifiable process.

The key generation specifies two ways to generate the private key stem:[x]:

* stem:[\mathrm{len}(q)]-bit output of the RBG, where stem:[1 \le x \le q-1]
* stem:[\mathrm{len}(q) + 64]-bit output of the RBG, followed by a stem:[\bmod (q-1)] operation and a stem:[+1] operation, where stem:[1 \le x \le q-1]
...
primes method and/or the group generator stem:[g] for a verifiable process, the
...
The evaluator shall verify the implementation of KCDSA Key Generation by the TOE using the Key Generation test. This test verifies the ability of the TSF to correctly produce values for the key components including the private signing exponent stem:[x], the public verification exponent stem:[y], the domain parameters stem:[p] and stem:[q], and the generator stem:[g].

FIPS 186-4 Key Pair generation specifies two methods for generating the primes stem:[p] and stem:[q].

These are:

* Primes stem:[p] and stem:[q] shall both be probable primes
* Primes stem:[p] and stem:[q] shall both be provable primes
...
* stem:[\mathrm{len}(q)]-bit output of the RBG, where stem:[2^{\mathrm{len}(q)-1} < x < q]
* stem:[\mathrm{len}(q) + 64]-bit output of the RBG, followed by a stem:[\bmod (q-1)] operation and a stem:[+1] operation, where stem:[0 < x < q]
...
. stem:[2^{\mathrm{len}(p)-1} < p < 2^{\mathrm{len}(p)}]
. stem:[2^{\mathrm{len}(q)-1} < q < 2^{\mathrm{len}(q)}]
. stem:[q \mid (p-1)]
. stem:[1 < g < p]
. stem:[g^q \bmod p = 1]
. stem:[0 < x < q]
. stem:[g^{x'} \bmod p = y], where stem:[x' = x^{-1} \bmod q]

*Key Generation for EC-KCDSA*

*_ECC Key Generation Test_*

For each selected curve, and for each key pair generation method as described in FIPS 186-4, section B.4, the evaluator shall require the implementation under test to generate 10 private/public key pairs stem:[(d, Q = d^{-1} G)], where stem:[G] is a base point. The private key, stem:[d], shall be generated using a random bit generator as specified in FCS_RBG_EXT.1. The private key, stem:[d], is used to compute the public key, stem:[Q']. The evaluator shall confirm that stem:[0 < d < n] (where stem:[n] is the order of the group), and the computed value stem:[Q'] is then compared to the generated public/private key pairs' public key, stem:[Q], to confirm that stem:[Q] is equal to stem:[Q'].

*_Public Key Validation (PKV) Test_*

For each supported curve, the evaluator shall generate 10 private/public key pairs using the key generation function of a known-good implementation and modify some public key values so that they are incorrect, leaving the other key values unchanged (i.e., correct). To determine correctness, the evaluator shall submit the 10 key pairs to the public key validation (PKV) function of the TOE and shall confirm that the results correspond as expected to the modified and unmodified values.
...
* Random seed stem:[d] [32 bytes]
* Random seed stem:[z] [32 bytes]

_Algorithm Functional Test_

For each supported parameter set the evaluator shall require the implementation under test to generate 25 key pairs using 25 different randomly generated pairs of 32-byte seed values stem:[(d, z)]. To determine correctness, the evaluator shall compare the resulting key pairs stem:[(\mathrm{ek}, \mathrm{dk})] with those generated using a known-good implementation using the same inputs.
...
The evaluator shall include test cases of FCS_CKM.1/SKG to the test subset. Note that FCS_CKM.1/SKG may not be mapped to the specific interface(s) after evaluator’s analysis during ADV_FSP.1.
...
calculation of the DLC primitives (the shared secret value stem:[Z]) and the
...
being incorrect: the shared secret value stem:[Z], the DKM, the other information
...
The evaluator shall verify the correctness of the TSF’s implementation of Diffie-Hellman group 14 by using a known-good implementation for each protocol selected in FTP_TRP.1/Admin, FTP_TRP.1/NonAdmin and FTP_ITC.1 that uses Diffie-Hellman group 14. 

*_FFC Schemes using “safe-prime” groups_*

The evaluator shall verify the correctness of the TSF’s implementation of safe-prime
...
*_Module-Lattice-Based Key-Encapsulation_*

The test activities that verify this claim are specified under
FCS_COP.1/KeyEncap.
...
The evaluator shall ensure that the TSS describes how IVs and tweaks are handled, as applicable based on the AES mode. 

The evaluator shall confirm that any applicable nonces, IVs, and tweaks meet the stated requirements.
...
Key stem:[i] in each set shall have the leftmost stem:[i] bits set to ones and the remaining bits to zeros, for values of stem:[i] from 1 to the key size. The keys and corresponding ciphertext are listed in AESAVS, Appendix E.
...
Plaintext value stem:[i] shall have the leftmost stem:[i] bits set to ones and the remaining bits set to zeros, for values of stem:[i] from 1 to 128. The plaintext values are listed in AESAVS, Appendix D.
...
The evaluator shall test the encrypt functionality by encrypting an stem:[i]-block message where stem:[1 < i \le 10]. The evaluator shall choose a key, an IV and plaintext message of length stem:[i] blocks and encrypt the message, using the mode to be tested, with the chosen key and IV. The ciphertext shall be compared to the result of encrypting the same plaintext message with the same key and IV using a known-good implementation.

The evaluator shall also test the decrypt functionality for each mode by decrypting an stem:[i]-block message where stem:[1 < i \le 10]. The evaluator shall choose a key, an IV and a ciphertext message of length stem:[i] blocks and decrypt the message, using the mode to be tested, with the chosen key and IV. The plaintext shall be compared to the result of decrypting the same ciphertext message with the same key and IV using a known-good implementation.
...
plaintext values for each selected key size and obtain the ciphertext value that
...
key values for each selected key size and obtain the ciphertext value that results
from encryption of an all zeros plaintext using the given key value.

KAT-3 To test the encrypt functionality, the evaluator shall supply a set of key values for each selected key size as described below and obtain the ciphertext values that result from AES encryption of an all-zeros plaintext using the given key values. A set of 128 128-bit keys, a set of 192 192-bit keys, and/or a set of 256 256-bit keys. stem:[\text{Key}_i] in each set shall have the leftmost stem:[i] bits be ones and the rightmost stem:[N-i] bits be zeros, for stem:[i \in [1, N\]].

KAT-4 To test the encrypt functionality, the evaluator shall supply the set of 128 plaintext values described below and obtain the ciphertext values that result from encryption of the given plaintext using each selected key size with a key value of all zeros (e.g., 256 ciphertext values will be generated if 128 bits and 256 bits are selected and 384 ciphertext values will be generated if all key sizes are selected). Plaintext value stem:[i] in each set shall have the leftmost stem:[i] bits be ones and the rightmost stem:[128-i] bits be zeros, for stem:[i \in [1, 128\]].

*AES-CTR Multi-Block Message Test*

The evaluator shall test the encrypt functionality by encrypting an stem:[i]-block message where stem:[1 < i \le 10] (test shall be performed using AES-ECB mode). For each stem:[i] the evaluator shall choose a key and plaintext message of length stem:[i] blocks and encrypt the message, using the mode to be tested, with the chosen key. The ciphertext shall be compared to the result of encrypting the same plaintext message with the same key using a known-good implementation. The evaluator shall perform this test using each selected key size.
...
test using each selected key size.
...
Key stem:[i] in each set shall have the leftmost stem:[i] bits set to ones and the remaining bits set to zeros, for values of stem:[i] from 1 to 128.
...
Plaintext value stem:[i] shall have the leftmost stem:[i] bits set to ones and the remaining bits set to zeros, for values of stem:[i] from 1 to 128.
...
The evaluator shall test the encrypt functionality by encrypting nine stem:[i]-block messages for a 128-bit key size, for stem:[2 \le i \le 10]. For each test, the evaluator shall supply a key, an IV, and a plaintext message of length stem:[i] blocks, and encrypt the message using SEED-CBC. The resulting ciphertext values shall be compared to the results of encrypting the plaintext messages using a known-good implementation.

The evaluator shall test the decrypt functionality by decrypting nine stem:[i]-block messages for a 128-bit key size, for stem:[2 \le i \le 10]. For each test, the evaluator shall supply a key, an IV, and a ciphertext message of length stem:[i] blocks, and decrypt the message using SEED-CBC. The resulting plaintext values shall be compared to the results of decrypting the ciphertext messages using a known-good implementation.
...
The ciphertext computed in the 1000th iteration (`CT[999]`) is the result for each of the 100 3-tuples for a 128-bit key. This result shall be compared to the result of running 1000 iterations with the same values using a known-good implementation.
...
IV value stem:[i] shall have the leftmost stem:[i] bits set to ones and the remaining bits set to zeros, for values of stem:[i] from 1 to 128.
...
for i = 0 to 99  {
    ...
    for j = 0 to 999 {
        if (j == 0) {
            CT[j] = SEED_CFB_ENCRYPT(Key[i], IV[i], PT[j])
            PT[j+1] = IV[i]
        } else {
            CT[j] = SEED_CFB_ENCRYPT(Key[i], PT[j])
            PT[j+1] = CT[j-1]
        }
    }
    Output CT[j]
...
The evaluator shall supply a single 3-tuple of pseudo-random values for a 128-bit key. This 3-tuple of plaintext, IV, and key is provided as input to the below algorithm to generate the remaining 99 3-tuples, and to run each 3-tuple through 1000 iterations of SEED-OFB encryption.
...
for i = 0 to 99  {
    ...
    for j = 0 to 999 {
        if (j == 0) {
            OT[j] = SEED-Encrypt(Key[i], IV[i])
            CT[j] = PT[j] xor OT[j]
            PT[j+1] = IV[i]
        } else {
            OT[j] = SEED-Encrypt(Key[i], OT[j-1])
            CT[j] = PT[j] xor OT[j]
            PT[j+1] = CT[j-1]
        }
    }
...
for i = 0 to 99  {
    Output Key[i], CTR[0], PT[0]
    for j = 0 to 999 {
        CT[j] = PT[j] XOR SEED-Encrypt(Key[i], CTR[0])
...
*Associated Data:* Two or three values for associated data length: The minimum (stem:[\ge 0] bytes) and maximum (stem:[\le 32] bytes) supported associated data lengths, and stem:[2^{16}] (65536) bytes, if supported.

*Payload:* Two values for payload length: The minimum (stem:[\ge 0] bytes) and maximum (stem:[\le 32] bytes) supported payload lengths.
...
Refer to SEED-CBC Multi-Block Message Tests for the required HIGHT-CBC testing. The evaluator shall test the encrypt functionality using the same test as SEED-CBC Tests, replacing `SEED-CBC-Encrypt` with `HIGHT-CBC-Encrypt`.

*_HIGHT-CBC Monte Carlo Tests_*

The evaluator shall test the encrypt functionality for each selected key size using 100 3-tuples of pseudo-random values for plaintext, IVs, and keys. 

The evaluator shall supply a single 3-tuple of pseudo-random values for 128-bits key. This 3-tuple of plaintext, IV, and key is provided as input to the below algorithm to generate the remaining 99 3-tuples, and to run each 3-tuple through 1000 iterations of HIGHT-CBC encryption.
...
for i = 0 to 99 {
    ...
    for j = 0 to 999 {
        if (j == 0) {
            CT[j] = HIGHT-CBC-Encrypt(Key[i], IV[i], PT[j])
            PT[j+1] = IV[i]
        } else {
            CT[j] = HIGHT-CBC-Encrypt(Key[i], PT[j])
            PT[j+1] = CT[j-1]
        }
    }
    Output CT[j]
...
The evaluator shall test the decrypt functionality using the same test as above, exchanging `CT` and `PT`, and replacing `HIGHT-CBC-Encrypt` with `HIGHT-CBC-Decrypt`.
...
The evaluator shall supply a single 3-tuple of pseudo-random values for 128-bits key. This 3-tuple of plaintext, IV, and key is provided as input to the below algorithm to generate the remaining 99 3-tuples, and to run each 3-tuple through 1000 iterations of HIGHT-CFB encryption.
...
for i = 0 to 99 {
    ...
    for j = 0 to 999 {
        if (j == 0) {
            CT[j] = HIGHT-CFB-Encrypt(Key[i], IV[i], PT[j])
            PT[j+1] = IV[i]
        } else {
            CT[j] = HIGHT-CFB-Encrypt(Key[i], PT[j])
            PT[j+1] = CT[j-1]
        }
    }
    Output CT[j]
...
The evaluator shall test the decrypt functionality using the same test as above, exchanging `CT` and `PT`, and replacing `HIGHT-CFB-Encrypt` with `HIGHT-CFB-Decrypt`.
...
The evaluator shall supply a single 3-tuple of pseudo-random values for 128-bits key. This 3-tuple of plaintext, IV, and key is provided as input to the below algorithm to generate the remaining 99 3-tuples, and to run each 3-tuple through 1000 iterations of HIGHT-OFB encryption.
...
for i = 0 to 99 {
    ...
    for j = 0 to 999 {
        if (j == 0) {
            OT[j] = HIGHT-Encrypt(Key[i], IV[i])
            CT[j] = PT[j] xor OT[j]
            PT[j+1] = IV[i]
        } else {
            OT[j] = HIGHT-Encrypt(Key[i], OT[j-1])
            CT[j] = PT[j] xor OT[j]
            PT[j+1] = CT[j-1]
        }
    }
...
The evaluator shall test the decrypt functionality using the same test as above, exchanging `CT` and `PT`, and replacing `HIGHT-OFB-Encrypt` with `HIGHT-OFB-Decrypt`.
...
The evaluator shall supply a single 3-tuple of pseudo-random values for 128-bits key. This 3-tuple of plaintext, counter, and key is provided as input to the below algorithm to generate the remaining 99 3-tuples, and to run each 3-tuple through 1000 iterations of HIGHT-CTR encryption.
...
for i = 0 to 99 {
    Output Key[i], CTR[0], PT[0]
    for j = 0 to 999 {
        CT[j] = PT[j] xor HIGHT-Encrypt(Key[i], CTR[0])
        CTR[0] = (CTR[0] + 1) mod 2^128
        PT[j+1] = CT[j]
    }
    Output CT[j]
...
The evaluator shall test the decrypt functionality using the same test as above, exchanging `CT` and `PT`, and replacing `HIGHT-CTR-Encrypt` with `HIGHT-CTR-Decrypt`.
...
Key stem:[i] in each set shall have the leftmost stem:[i] bits set to ones and the remaining bits set to zeros, for values of stem:[i] from 1 to the key size.
...
The evaluator shall test the encrypt functionality by encrypting nine stem:[i]-block messages for each selected key size, for stem:[2 \le i \le 10]. For each test, the evaluator shall supply a key, an IV, and a plaintext message of length stem:[i] blocks, and encrypt the message using LEA-CBC. The resulting ciphertext values shall be compared to the results of encrypting the plaintext messages using a known-good implementation.

The evaluator shall test the decrypt functionality by decrypting nine stem:[i]-block messages for each selected key size, for stem:[2 \le i \le 10]. For each test, the evaluator shall supply a key, an IV, and a ciphertext message of length stem:[i] blocks, and decrypt the message using LEA-CBC. The resulting plaintext values shall be compared to the results of decrypting the ciphertext messages using a known-good implementation.

*_LEA-CBC Monte Carlo Tests_*

The evaluator shall test the encrypt functionality for each selected key size using 100 3-tuples of pseudo-random values for plaintext, IVs, and keys.

The evaluator shall supply a single 3-tuple of pseudo-random values for each selected key size. This 3-tuple of plaintext, IV, and key is provided as input to the below algorithm to generate the remaining 99 3-tuples, and to run each 3-tuple through 1000 iterations of LEA-CBC encryption.
...
*KAT-1 (Variable Key):* To test the encrypt functionality of LEA-CFB, the evaluator shall supply a set of keys for each selected key size (as described below) and obtain the ciphertext value that results from LEA encryption of an all-zeros plaintext using each key and an IV of all zeros.
...
for i = 0 to 99 {
    ...
    for j = 0 to 999 {
        if (j == 0) {
            CT[j] = LEA-CFB-Encrypt(Key[i], IV[i], PT[j])
            PT[j+1] = IV[i]
        } else {
            CT[j] = LEA-CFB-Encrypt(Key[i], PT[j])
            PT[j+1] = CT[j-1]
        }
    }
    Output CT[j]
...
The evaluator shall supply a single 3-tuple of pseudo-random values for each selected key size. This 3-tuple of plaintext, IV, and key is provided as input to the below algorithm to generate the remaining 99 3-tuples, and to run each 3-tuple through 1000 iterations of LEA-OFB encryption.
...
for i = 0 to 99 {
    ...
    for j = 0 to 999 {
        if (j == 0) {
            OT[j] = LEA-Encrypt(Key[i], IV[i])
            CT[j] = PT[j] xor OT[j]
            PT[j+1] = IV[i]
        } else {
            OT[j] = LEA-Encrypt(Key[i], OT[j-1])
            CT[j] = PT[j] xor OT[j]
            PT[j+1] = CT[j-1]
        }
    }
...
The evaluator shall supply a single 3-tuple of pseudo-random values for each selected key size. This 3-tuple of plaintext, counter, and key is provided as input to the below algorithm to generate the remaining 99 3-tuples, and to run each 3-tuple through 1000 iterations of LEA-CTR encryption.
...
for i = 0 to 99 {
    Output Key[i], CTR[0], PT[0]
    for j = 0 to 999 {
        CT[j] = PT[j] xor LEA-Encrypt(Key[i], CTR[0])
        CTR[0] = (CTR[0] + 1) mod 2^128
        PT[j+1] = CT[j]
    }
    Output CT[j]
...
The testing for LEA-CCM consists of five tests. To determine correctness in each of the below tests, the evaluator shall compare the ciphertext with the result of encryption of the same inputs with a known-good implementation.
...
The evaluator shall include test cases of FCS_COP.1/SigGen to the test subset. Note that FCS_COP.1/SigGen may not be mapped to the specific interface(s) after evaluator’s analysis during ADV_FSP.1.
...
For each supported NIST curve (i.e., P-256, P-384 and P-521) and SHA
function pair, the evaluator shall generate a set of 10 1024-bit messages, public
...
For each modulus size/hash algorithm selected, the evaluator generates a modulus and three associated key pairs, stem:[(d, e)]. Each private key stem:[d] is used to sign six pseudorandom messages each of 1024 bits using a trusted reference implementation of the signature generation algorithm. Some of the public keys, stem:[e], messages, or signatures are altered so that signature verification should fail. For both the set of original messages and the set of altered messages: the modulus, hash algorithm, public key stem:[e] values, messages, and signatures are forwarded to the TOE, which then attempts to verify the signatures and returns the verification results.
...
For each supported NIST curve (i.e., P-224/P-256, B-233/B-283, K-233/K-283) and SHA function pair, the evaluator shall generate a set of 10 1024-bit messages. The test passes only if all the signatures with the submitted parameters result in successful signature verification.

*_Signature Verification Test_*

The EC-KCDSA Signature Verification Test tests the ability of the TSF to recognize valid and invalid signatures. The evaluator shall provide a base point and associated key pair stem:[(x, y)] for each combination of selected curve and hash size. Each private key stem:[x] is used to sign 15 pseudorandom messages of 1024 bits. For eight of the fifteen messages, the message, IR format, padding, or signature is altered so that signature verification should fail. The test passes only if all the signatures made using unaltered parameters result in successful signature verification, and all the signatures made using altered parameters result in unsuccessful signature verification.
...
* For one test case, modify the signature, i.e., the signature is different.
...
*_XMSS Digital Signature Verification Test_*
...
For each combination of supported parameter set and capabilities, the evaluator shall require the implementation under test to generate 15 signature pairs using 15 different randomly generated 32-byte seed values. To determine correctness, the evaluator shall compare the resulting key pairs with those generated using a known-good implementation using the same inputs.

*_Known Answer Test for Rejection Cases_*
...
*_ML-DSA-87 Test Cases for Rejection Cases_*
...
*_Known Answer Test for Large Number of Rejection Cases (Total Rejection Count)_*
...
*_ML-DSA-87 Test Cases for Total Rejection Count_*
...
The evaluators devise an input set consisting of stem:[m+1] messages, where stem:[m] is the block length of the hash algorithm. The lengths of the messages range sequentially from 0 to stem:[m] bits. The message text shall be pseudorandomly generated. The evaluators compute the message digest for each of the messages and ensure that the correct result is produced when the messages are provided to the TSF.

*Short Messages Test - Byte-oriented Mode*

The evaluators devise an input set consisting of stem:[m/8+1] messages, where stem:[m] is the block length of the hash algorithm. The lengths of the messages range sequentially from 0 to stem:[m/8] bytes, with each message being an integral number of bytes. The message text shall be pseudorandomly generated. The evaluators compute the message digest for each of the messages and ensure that the correct result is produced when the messages are provided to the TSF.

*Selected Long Messages Test - Bit-oriented Mode*

The evaluators devise an input set consisting of stem:[m] messages, where stem:[m] is the block length of the hash algorithm (e.g., 512 bits for SHA-256). The length of the stem:[i]th message is stem:[m + 99i], where stem:[1 \le i \le m]. The message text shall be pseudorandomly generated. The evaluators compute the message digest for each of the messages and ensure that the correct result is produced when the messages are provided to the TSF.

*Selected Long Messages Test - Byte-oriented Mode*

The evaluators devise an input set consisting of stem:[m/8] messages, where stem:[m] is the block length of the hash algorithm (e.g., 512 bits for SHA-256). The length of the stem:[i]th message is stem:[m + 8 \cdot 99 \cdot i], where stem:[1 \le i \le m/8]. The message text shall be pseudorandomly generated. The evaluators compute the message digest for each of the messages and ensure that the correct result is produced when the messages are provided to the TSF.

*Pseudorandomly Generated Messages Test*

This test is for byte-oriented implementations only. The evaluators randomly generate a seed that is stem:[n] bits long, where stem:[n] is the length of the message digest produced by the hash function to be tested. The evaluators then formulate a set of 100 messages and associated digests by following the algorithm provided in Figure 1 of [SHAVS]. The evaluators then ensure that the correct result is produced when the messages are provided to the TSF.
...
The evaluator shall include test cases of FCS_RBG_EXT.1 to the test subset. Note that FCS_RBG_EXT.1 may not be mapped to the specific interface(s) after evaluator’s analysis during ADV_FSP.1.
...
*Personalization string:* The length of the personalization string must be stem:[\le] seed length. If the implementation only supports one personalization string length, then the same length can be used for both values. If more than one string length is supported, the evaluator shall use personalization strings of two different lengths. If the implementation does not use a personalization string, no value needs to be supplied.
...
Testing for this SFR is performed in the tests of FMT_MOF.1, FMT_MSA.1, and FMT_MTD.1.
...
. For every element in each chain of trust, the evaluator shall attempt to boot the TOE using firmware/software with an invalid hash, digital signature, or message authentication verification and verify that the verification check fails and the TOE does not execute corrupted firmware/software.
+
...
.. (conditional) If ‘revert to previous TOE image’ is selected, the evaluator, following a failed boot attempt, shall review the guidance documentation, verify that the TOE performed the action of reverting to a previous TOE image and confirm that the TOE returns to an operational state following the remediation action.
+
Note: The administrator might need to take an action to perform the remediation action.

.. (conditional) If ‘reinstall TOE image’ is selected, the evaluator, following a failed boot attempt, shall review the guidance documentation, verify that the TOE performed the action of reinstalling the TOE image and confirm that the TOE returns to an operational state following the remediation action.
+
Note: The administrator might need to take an action to perform the remediation action.

.. (conditional) If ‘perform factory reset’ is selected, the evaluator, following a failed boot attempt, shall review the guidance documentation, verify that the TOE performs a factory reset and confirm that the TOE returns to an initialized state where it can be returned into an operational state following the remediation action.
+
Note: The administrator might need to take an action to perform the remediation action.
.. (conditional) If ‘indicate a need to contact vendor support’ is selected, the evaluator, following a failed boot attempt, shall review the guidance documentation and verify that the TSF provides an indication to contact vendor support.
...
_Test activities consist of the six tests below. Test 2 and 6 are mandatory in all cases. Tests 3, 4, and 5 are conditional on the wipe method selected by the ST author._
...
Key stem:[i] in each set shall have the leftmost stem:[i] bits set to ones and the remaining bits set to zeros, for values of stem:[i] from 1 to the key size. The keys and corresponding ciphertext
...
Plaintext value stem:[i] shall have the leftmost stem:[i] bits set to ones and the remaining bits set to zeros, for values of stem:[i] from 1 to 128. The plaintext values are listed in AESAVS,
Appendix D.
...
* The largest data length supported by the implementation, or stem:[2^{16}] (65536), whichever is larger.
...
The evaluator shall include test cases of FCS_COP.1/KeyWrap to the test subset. Note that FCS_COP.1/KeyWrap may not be mapped to the specific interface(s) after evaluator’s analysis during ADV_FSP.1.
...
The evaluator shall perform the Algorithm Functional Tests and Additional Negative Tests  of SEED-KW for each combination of the following input parameters:
...
The evaluator shall perform the Algorithm Functional Tests and Additional Negative Tests of SEED-KWP for each combination of the following input parameters:
...
For each set of the above parameters the evaluator shall generate a set of 100 key and plaintext pairs and obtain the ciphertext that results from SEED-KWP authenticated encryption. To determine correctness, the evaluator shall compare the results with those obtained from the SEED-KWP authenticated-encryption function of a known-good implementation.
...
Generate one ciphertext using algorithm KWP-AE with substring stem:[[\mathrm{len}(P)/8\]_{32}] of S replaced by each of the following 32-bit values, where stem:[\mathrm{len}(P)] is the length of P in bits and stem:[[\,\]_{32}] denotes representation of an integer in 32 bits:

. stem:[[0\]_{32}]
. stem:[[\mathrm{len}(P)/8-8\]_{32}]
. stem:[[\mathrm{len}(P)/8+8\]_{32}]
. stem:[[513\]_{32}]
...
Refer to SEED-KW [SP 800-38F, sec. 6.2] for the required LEA-KW testing. The evaluator shall perform the Algorithm Functional Tests and Additional Negative Tests of LEA-KW for each combination of the following input parameters:

1.	All supported and selected key sizes (e.g., 128, 192, 256 bits).
...
For each set of the above parameters the evaluator shall generate a set of 100 key and plaintext pairs and obtain the ciphertext that results from LEA-KW authenticated encryption. To determine correctness, the evaluator shall compare the results with those obtained from the LEA-KW authenticated-encryption function of a known-good implementation.

The evaluator shall test the authenticated-decryption functionality of LEA-KW using the same test as for authenticated-encryption, replacing plaintext values with ciphertext values and LEA-KW authenticated-encryption (KW-AE) with LEA-KW authenticated-decryption (KW-AD). For the authenticated-decryption test, 20 out of the 100 trials per plaintext length must have ciphertext values that are not authentic; that is, they fail authentication

Additionally, the evaluator shall perform the following negative tests:
	
[.underline]#Test 1 (Invalid Ciphertext Length Test)#
	
Determine the valid ciphertext lengths of the implementation from the TOE specification. Verify that the implementation of KW-AD in the TOE rejects ciphertexts of invalid length by testing ciphertext of the following lengths:

1.	Plaintext length greater than 64 semi-blocks
2.	Plaintext bit-length not divisible by 64
3.	Plaintext with length 0, and 4) plaintext with one semi-block.

[.underline]#Test 2 (Invalid Ciphertext Length Test)#

Determine the valid ciphertext lengths of the implementation from the TOE specification. Verify that the implementation of KW-AD in the TOE rejects ciphertexts of invalid length by testing ciphertext of the following lengths:

1.	Ciphertext with length greater than 65 semi-blocks
2.	Ciphertext with bit-length not divisible by 64
3.	Ciphertext with length 0
4.	Ciphertext with length of one semi-block
5.	Ciphertext with length of two semi-blocks

[.underline]#Test 3 (Invalid ICV1 Test)#

Test that the implementation detects invalid ICV1 values by encrypting any plaintext value eight times using a different value for ICV1 each time as follows: 

1.	Start with a base ICV1 of 0xA6A6A6A6A6A6A6A6
2.	For each of the eight tests change a different byte to a different value, so that each of the eight bytes is changed once

Verify that the implementation of KW-AD in the TOE outputs FAIL for each test.

*LEA-KWP [SP 800-38F, sec. 6.3]*

Refer to SEED-KW [SP 800-38F, sec. 6.3] for the required LEA-KWP testing. The evaluator shall perform the Algorithm Functional Tests and Additional Negative Tests of LEA-KWP for each combination of the following input parameters:
...
For each set of the above parameters the evaluator shall generate a set of 100 key and plaintext pairs and obtain the ciphertext that results from LEA-KWP authenticated encryption. To determine correctness, the evaluator shall compare the results with those obtained from the LEA-KWP authenticated-encryption function of a known-good implementation.

The evaluator shall test the authenticated-decryption (KWP-AD) functionality of LEA-KWP using the same test as for authenticated-encryption, replacing plaintext values with ciphertext values and LEA-KWP authenticated-encryption with LEA-KWP authenticated-decryption. For the authenticated-decryption test, 20 out of the 100 trials per plaintext length must have ciphertext values that are not authentic; that is, they fail authentication.

Additionally, the evaluator shall perform the following negative tests:
	
[.underline]#Test 1 (Invalid Plaintext Length Test)#
	
Determine the valid plaintext lengths of the implementation from the TOE specification. Verify that the implementation of KWP-AE in the TOE rejects plaintexts of invalid length by testing plaintext of the following lengths:

. Plaintext length greater than 64 semi-blocks
. Plaintext bit-length not divisible by 8
. Plaintext with length 0

[.underline]#Test 2 (Invalid Ciphertext Length Test)#

Determine the valid ciphertext lengths of the implementation from the TOE specification. Verify that the implementation of KWP-AD in the TOE rejects ciphertexts of invalid length by testing ciphertext of the following lengths:

. Ciphertext with length greater than 65 semi-blocks
. Ciphertext with bit-length not divisible by 64
. Ciphertext with length 0
. Ciphertext with length of one semi-block

[.underline]#Test 3 (Invalid ICV2 Test)#

Test that the implementation detects invalid ICV2 values by encrypting any plaintext value eight times using a different value for ICV2 each time as follows:

1.	Start with a base ICV2 of 0xA65959A6
2.	For each of the four tests change a different byte to a different value, so that each of the four bytes is changed once
3.	Verify that the implementation of KWP-AD in the TOE outputs FAIL for each test.

[.underline]#Test 4 (Invalid Padding Length Test)#
...
[.underline]#Test 5 (Invalid Padding Bits Test)#
...
For each supported key transport scheme, the evaluator shall initiate at least 25 sessions that require key transport with an independently developed remote instance of a key transport entity, using known RSA key-pairs. The evaluator shall observe traffic passed from the sender-side and to the receiver-side of the TOE, and shall perform the following tests, specific to which key transport scheme was employed.

If the KTS-OAEP scheme was selected, the evaluator shall perform the following tests:

. The evaluator shall inspect each ciphertext, stem:[C], produced by the RSA-OAEP encryption operation of the TOE and make sure it is the correct length, either 256 or 384 bytes depending on RSA key size. The evaluator shall also feed into the TOE's RSA-OAEP decryption operation some ciphertexts that are the wrong length and verify that the erroneous input is detected and that the decryption operation exits with an error code.

. The evaluator shall convert each ciphertext, stem:[C], produced by the RSA-OAEP encryption operation of the TOE to the correct ciphertext integer, stem:[c], and use the decryption primitive to compute stem:[em = \mathrm{RSADP}((n,d), c)] and convert stem:[em] to the encoded message EM. The evaluator shall then check that the first byte of EM is `0x00`. The evaluator shall also feed into the TOE's RSA-OAEP decryption operation some ciphertexts where the first byte of EM was set to a value other than `0x00`, and verify that the erroneous input is detected and that the decryption operation exits with an error code.

. The evaluator shall decrypt each ciphertext, stem:[C], produced by the RSA-OAEP encryption operation of the TOE using RSADP, and perform the OAEP decoding operation (described in SP 800-56B, section 7.2.2.4) to recover stem:[HA' \| X]. For each stem:[HA'], the evaluator shall take the corresponding stem:[A] and the specified hash algorithm and verify that stem:[HA' = \mathrm{Hash}(A)]. The evaluator shall also force the TOE to perform some RSA-OAEP decryptions where the stem:[A] value is passed incorrectly, and the evaluator shall verify that an error is detected.

. The evaluator shall check the format of the "X" string recovered in OAEP Test 3 to ensure that the format is of the form stem:[PS \| 01 \| K], where stem:[PS] consists of zero or more consecutive `0x00` bytes and stem:[K] is the transported keying material. The evaluator shall also feed into the TOE's RSA-OAEP decryption operation some ciphertexts for which the resulting "X" strings do not have the correct format (i.e., the leftmost non-zero byte is not `0x01`). These incorrectly formatted "X" variables shall be detected by the RSA-OAEP decrypt function.
...
. The evaluator shall inspect each ciphertext, stem:[C], produced by the RSA-KEM-KWS encryption operation of the TOE and make sure the length (in bytes) of the ciphertext, stem:[cLen], is greater than stem:[nLen] (the length, in bytes, of the modulus of the RSA public key) and that stem:[cLen - nLen] is consistent with the byte lengths supported by the key wrapping algorithm. The evaluator shall feed into the RSA-KEM-KWS decryption operation a ciphertext of unsupported length and verify that an error is detected and that the decryption process stops.

. The evaluator shall separate the ciphertext, stem:[C], produced by the sender-side of the TOE into its stem:[C_0] and stem:[C_1] components and use the RSA decryption primitive to recover the secret value, stem:[Z], from stem:[C_0]. The evaluator shall check that the unsigned integer represented by stem:[Z] is greater than 1 and less than stem:[n-1], where stem:[n] is the modulus of the RSA public key. The evaluator shall construct examples where the ciphertext is created with a secret value stem:[Z = 1] and make sure the RSA-KEM-KWS decryption process detects the error. Similarly, the evaluator shall construct examples where the ciphertext is created with a secret value stem:[Z = n-1] and make sure the RSA-KEM-KWS decryption process detects the error.

. The evaluator shall attempt to successfully recover the secret value stem:[Z], derive the key wrapping key, stem:[KWK], and unwrap the KWA-ciphertext following the RSA-KEM-KWS decryption process given in SP 800-56B, section 7.2.3.4. If the key-wrapping algorithm is AES-CCM, the evaluator shall verify that the value of any (unwrapped) associated data, stem:[A], that was passed with the wrapped keying material is correct. The evaluator shall feed into the TOE's RSA-KEM-KWS decryption operation examples of incorrect ciphertext and verify that a decryption error is detected. If the key-wrapping algorithm is AES-CCM, the evaluator shall attempt at least one decryption where the wrong value of stem:[A] is given to the RSA-KEM-KWS decryption operation and verify that a decryption error is detected. Similarly, if the key-wrapping algorithm is AES-CCM, the evaluator shall attempt at least one decryption where the wrong nonce is given to the RSA-KEM-KWS decryption operation and verify that a decryption error is detected.

. The evaluator shall trigger all detectable decryption errors and validate that the resulting error codes are the same and that no information is given back to the sender on which type of error occurred. The evaluator shall also validate that no intermediate results from the TOE's receiver-side operations (in particular, no stem:[Z] values) are revealed to the sender.
...
* Test 1.a: If “according to the security strength associated with the negotiated Diffie-Hellman group” has been selected, for each supported authentication methods and DH
...
For efficiency, the testing is combined with the testing for FIA_X509_EXT.1, FIA_X509_EXT.2 (for IPsec connections), and FCS_IPSEC_EXT.1.1.
...
.. Identification of the hardware components (if any) that form the basis of the argument.
...
** Parameter set [ML-KEM-1024]
** Previously generated encapsulation key (`ek`) [1568 bytes]
** Random value (`m`) [32 bytes]
* Decapsulation Parameters:
** Parameter set [ML-KEM-1024]
** Previously generated decapsulation key (`dk`) [3168 bytes]
** Previously generated ciphertext (`c`) [1568 bytes]

*Encapsulation Test*

For each supported parameter set the evaluator shall generate 25 test cases
consisting of an encapsulation key `ek` and random value `m`. For each test case
the evaluator shall require the implementation under test to generate the
...
. The evaluator shall compose at least 15 pre-shared keys of 22 characters that cover all allowed characters in various combinations that conform to the operational guidance, and demonstrate that a successful protocol negotiation can be performed with each key.
...
The evaluator shall include test cases of FCS_CKM.5 to the test subset. Note that FCS_CKM.5 may not be mapped to the specific interface(s) after evaluator’s analysis during ADV_FSP.1.

The evaluator shall produce test documentation for test cases of FCS_CKM.5. If there is no explicit external interface(s) mapped to FCS_CKM.5, the evaluator shall employ an alternative test approach (refer to CEM, section 15.2.2.).
...
. One or more pseudorandom functions (PRFs) that are included in the "cryptographic key derivation algorithm" selection in the SFR, and their output lengths in bits stem:[(h)]
. One or more of the values stem:[\{8, 16, 24, 32\}] that equal the length of the binary representation of the counter stem:[(r)], and the location of the counter relative to the fixed input data: before, after, or in the middle. If the counter is in the middle, then the lengths of data before and after the counter must be determined.
. The "cryptographic key sizes" selections in the SFR, i.e., the lengths (in bits) of the derived keying material stem:[(L)]

For each supported combination of PRF, counter location, value of stem:[r], and value of stem:[L], the evaluator shall generate 20 pseudorandom key derivation key values stem:[(K_I)].

For each value of stem:[K_I], the evaluator shall supply this data to the TOE in order to produce the keying material output stem:[K_O]. The evaluator shall verify that the resulting output matches the results from submitting the same inputs to a known-good implementation of the key derivation function, having the same characteristics.
...
. One or more pseudorandom functions (PRFs) that are included in the "cryptographic key derivation algorithm" selection in the SFR, and their output lengths in bits stem:[(h)]
. If the implementation includes a counter, then one or more of the values stem:[\{8, 16, 24, 32\}] that equal the length of the binary representation of the counter stem:[(r)], and the location of the counter relative to the fixed input data: before, after, or in the middle. If the counter is in the middle, then the lengths of data before and after the counter must be determined.
. The "cryptographic key sizes" selections in the SFR, i.e., the lengths (in bits) of the derived keying material stem:[(L)]
. The supported IV lengths.

For each supported combination of PRF, counter location (if a counter is used), value of stem:[r] (if a counter is used), value of stem:[L], and IV length, the evaluator shall generate 20 pseudorandom key derivation key values stem:[(K_I)].
...
For each supported combination of PRF, counter location (if a counter is used), value of stem:[r] (if a counter is used), and value of stem:[L], the evaluator shall generate 20 pseudorandom key derivation key values stem:[(K_I)].
...
For each of the supported parameter sets, the evaluator shall compose at least 15 sets of test data. Each set shall consist of a key and message data. The test data shall include messages of different lengths, some with partial blocks as the last block and some with full blocks as the last block. The test data keys shall include cases for which subkey stem:[K_1] is generated both with and without using the irreducible polynomial stem:[R_b], as well as cases for which subkey stem:[K_2] is generated from stem:[K_1] both with and without using the irreducible polynomial stem:[R_b]. (The subkey generation and polynomial stem:[R_b] are as defined in SP 800-38B.) The evaluator shall have the TSF generate CMAC tags for these sets of test data. The resulting MAC tags shall be compared to the result of generating CMAC tags with the same key using a known-good implementation.

==== FCS_SNI_EXT.1 Cryptographic Operation (Salt and Nonce Generation)

===== TSS

The evaluator shall ensure the TSS describes how salts are generated. The evaluator shall confirm that the salt is generated using an RBG described in FCS_RBG_EXT.1.

The evaluator shall ensure the TSS describes how nonces are created uniquely. The evaluator shall confirm that the nonces are unique.
...
The evaluator shall demonstrate that checking the validity of a certificate is performed when a certificate is used in an authentication step or when performing trusted updates. It is not sufficient to verify the status of an X.509 certificate only when it is loaded onto the TOE. It is not necessary to verify the revocation status of X.509 certificates during power-up self-tests (if the option for using X.509 certificates for self-testing is selected). The evaluator shall perform the following tests for FIA_X509_EXT.1. These tests must be repeated for each distinct security function that utilizes X.509v3 certificates. For example, if the TOE implements certificate-based authentication with IPsec and TLS, then it shall be tested with each of these protocols:
...
The evaluator shall examine the TSS to confirm that it describes the behaviour of the TOE when a connection cannot be established during the validity check of a certificate used in establishing a trusted channel. The evaluator shall verify that any distinctions between trusted channels are described. If the administrator is able to specify the default action, then the evaluator shall ensure that the guidance documentation contains instructions on how this configuration action is performed.
...
|AVA_VAN.1-2 The evaluator *__shall examine__* the TOE to determine that it has been installed properly and is in a known state. 
|The evaluator shall perform the CEM activity as specified.

|AVA_VAN.1-3 The evaluator *__shall examine__* sources of information publicly available to identify potential vulnerabilities in the TOE. 
...
. Instructions to connect and set up all required test equipment as required to conduct the penetration test;
...
* [#CPP_FDE_EE_V2.0]#[CPP_FDE_EE_V2.0]# collaborative Protection Profile for Full Drive Encryption - Encryption Engine, Version 2.0 September 09, 2016
...
. A list of flaw hypotheses derived from information available to the evaluators; this includes  the baseline evidence provided by the vendor described in this Supporting Document (documentation associated with EAs, documentation described in <<Evaluation Activity (Documentation)>>, documentation described in <<Required Supplementary Information>>), as well as other information (public and/or based on evaluator experience) as documented in <<Type3Hypotheses>>; and
...
functionality provided by the library affects cPP-specified
...
model variation is identical, the model variations
shall be considered equivalent.
|Different
|If the functionality provided by different TOE
model variations differ, a determination must be
made if the functional differences affect cPP-specified
...
Environment in which the TOE is operated.[CC1]

Protection Profile::
Implementation-independent statement of security needs for a TOE type. [CC1]
...
A description of how assurance is to be gained that the TOE meets the SFRs. [CC1]

Security Functional Requirement::
A translation of the Security Objectives for the TOE into a standardized language. [CC1]

Security Objective::
Statement of an intent to counter identified Threats and/or satisfy identified organization security policies and/or Assumptions. [CC1]

Security Target::
Implementation-dependent statement of security needs for a specific identified TOE. [CC1]

Submask::
A submask is a bit string that can be generated and stored in a number of ways, such as passphrases, tokens, etc. [CPP_FDE_EE_V2.0]

Target of Evaluation::
Set of software, firmware and/or hardware possibly accompanied by guidance. [CC1]

TOE Security Functionality::
Combined functionality of all hardware, software, and firmware of a TOE that must be relied upon for the correct enforcement of the SFRs. [CC1]

TSF Data::
Data for the operation of the TOE upon which the enforcement of the SFR relies. [CC2]
...
User Data::
Data for the User that does not affect the operation of the TSF. [CC2]
...
|AGD      | Administrator Guidance Documentation
